...
sftp=ssh.open_sftp()
for f in ["app/affiliate/commissions/page.tsx","app/affiliate/withdraw/page.tsx"]:
    sftp.put(LB+"\\"+f.replace("/","\\"), A+"/"+f)
sftp.close(); print("### uploaded commissions + withdraw")
...
if done:
    ...
    out,_=run("for u in /affiliate/commissions /affiliate/withdraw; do echo \"$u $(curl -s -m12 -o /dev/null -w '%{http_code}' https://autopartsvietnam.com.vn$u)\"; done; pm2 describe autoparts 2>/dev/null|grep status|head -1")
    print("### routes:\n"+out.strip())
else:
