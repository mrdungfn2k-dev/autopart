...
function VINLookupInner() {
  ...
  const handleLookup = (overrideVin?: string) => {
    const searchVin = typeof overrideVin === "string" ? overrideVin : vin;
    setError("");
    setResult(null);
    ...
    setTimeout(() => {
      setLoading(false);
      const found = sampleVINs[searchVin.toUpperCase()];
      if (found) setResult(found);
      else setError(lang === "zh" ? "找不到此VIN码的车辆信息。请仔细检查或按品牌搜索。" : "Không tìm thấy thông tin xe với mã VIN này. Vui lòng kiểm tra lại hoặc tìm theo hãng xe.");
    }, 1200);
  };
