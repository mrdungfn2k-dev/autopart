...
export default function AdminFinancePage() {
  ...
  const statusLabel: Record<string, string> = { PAID: (lang === "zh" ? "已结算" : "Đã giải ngân"), PENDING: (lang === "zh" ? "处理中" : "Đang xử lý"), SCHEDULED: "Lên lịch" };
 ...
 useEffect(() => {
   fetch("/api/payouts").then(r => r.json()).then(data => {
     if (Array.isArray(data)) {
       ...
       setCommissions(data.filter((p: {type:string}) => p.type === "affiliate").map((p: any) => ({ name: p.name, role: p.role ?? "T1", amount: p.amount, date: p.date, status: p.status, id: p.id })));
     }
   }).catch(() => {});
 }, []);
 ...
 const approveCommission = async (id: string, name: string) => {
   ...
   showToast("Đã duyệt hoa hồng cho " + name);
 };
 ...
 <AdminSidebar active="/admin/finance" />
 ...
 {([ ["overview", " Tổng quan"], ["supplier", "Giải ngân NCC"], ["affiliate", " Hoa hồng Đại lý"] ] as [string,string][]).map(([key, label]) => (
 ...
 {[
 { label: "Doanh thu nền tảng tháng này", value: "250M", color: "var(--ap-primary)", icon: "" },
 { label: "Tổng giải ngân NCC (chờ)", value: "127.3M", color: "var(--ap-primary)", icon: "" },
 { label: "Hoa hồng đại lý (chờ)", value: "29.8M", color: "#8B5CF6", icon: "" },
 ...
 <YAxis hide />
 <Tooltip formatter={(v) => [`${Number(v)}M`, ""]} contentStyle={{ borderRadius: "8px", border: "1px solid #E2E8F0" }} />
 ...
 <Line type="monotone" dataKey="gmv" stroke="var(--ap-primary)" strokeWidth={2.5} dot={{ fill: "var(--ap-primary)", r: 4 }} name="GMV (Tỷ)" />
 </LineChart>
 </ResponsiveContainer>
 </div>
 ...
 <tbody>
 {commissions.map(c => (
 <tr key={c.name} className="border-t border-slate-50 hover:bg-[#f8f8fa]">
 <td className="px-4 py-3 font-semibold text-[#44494d]">{c.name}</td>
 <td className="px-4 py-3">
 ...
 <td className="px-4 py-3 font-bold text-purple-600">{fp(c.amount)}</td>
 ...
 <td className="px-4 py-3">
 {c.status === "PENDING" && <button onClick={() => approveCommission((c as any).id ?? c.name, c.name)} className="px-3 py-1 rounded-lg text-xs font-bold text-white" style={{ background: "#22C55E" }}>Duyệt giải ngân</button>}
 {c.status !== "PENDING" && <span className="text-xs text-[#8f9294]">—</span>}
 </td>
 </tr>
 ))}
 </tbody>
