...
export default function SupplierInventoryPage() {
  ...
  const inventoryData = products.map(p => {
    ...
    return {
      ...
      minStock: 10,
      lastRestocked: `${String((seed % 28) + 1).padStart(2, "0")}/10/2024`,
    };
  });
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden" style={{ background: "var(--ap-page-bg)" }}>
      <SupplierSidebar active="/supplier/inventory" />
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6">
          ...
          {lowStockItems.length > 0 && (
            <div className="flex items-start gap-3 p-4 rounded-xl mb-5" style={{ background: "#FEF3C7", border: "1px solid #FCD34D" }}>
              ...
              <div>
                <p className="font-bold text-yellow-800">Cảnh báo: {lowStockItems.length} sản phẩm sắp hết hàng!</p>
                <p className="text-yellow-700 text-sm mt-0.5">
                  {lowStockItems.slice(0, 3).map(p => p.name.split(" ").slice(0, 3).join(" ")).join(", ")}
                  {lowStockItems.length > 3 ? ` và ${lowStockItems.length - 3} sản phẩm khác` : ""}. Hãy nhập kho sớm.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {[
              { label: "Tổng SKU", value: inventoryData.length.toString(), color: "#44494d" },
              { label: "Tổng tồn kho", value: inventoryData.reduce((s, p) => s + (p.stock || 0), 0).toLocaleString(), color: "var(--ap-primary)" },
              { label: "Sắp hết hàng", value: lowStockItems.length.toString(), color: "#F59E0B" },
              { label: "Hàng đang về", value: inventoryData.filter(p => p.incoming > 0).length.toString(), color: "#22C55E" },
            ].map(s => (
          ...
          {/* Filters */}
          <div className="bg-white rounded-xl border border-[#f0f0f0] p-4 mb-4 flex flex-wrap gap-3">
            ...
            <div className="flex rounded-lg border border-[#e5e5e5] overflow-hidden text-sm">
              {(["all", "low", "ok"] as const).map((k, i) => {
                ...
                return (
                  <button key={k} onClick={() => setFilterStock(k)}
                    ...
                    style={filterStock === k ? { background: "var(--ap-primary)" } : {}}>
                    {labels[i]} ({counts[i]})
                  </button>
                );
              })}
            ...
            <table className="w-full">
              <thead style={{ background: "#f8f8fa" }}>
                <tr className="text-xs text-[#8f9294] font-semibold">
                  {["SẢN PHẨM", "VỊ TRÍ KHO", "TỒN KHO", "HÀNG VỀ", "NHẬP LẦN CUỐI", "GIÁ VỐN", "TRẠNG THÁI", ""].map(h => (
                    ...
                    <tr key={prod.id} className={`border-t border-slate-50 transition-colors ${prod.stock < prod.minStock ? "bg-yellow-50/30" : "hover:bg-[#f8f8fa]"}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-[#44494d] text-sm line-clamp-1">{prod.name}</p>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-lg font-bold ${prod.stock < prod.minStock ? "text-red-500" : "text-[#44494d]"}`}>{prod.stock}</span>
                        {prod.stock < prod.minStock && <span className="text-xs text-red-500 font-bold ml-1">!</span>}
                      </td>
                      <td className="px-4 py-3">
                        {prod.incoming > 0
                          ? <span className="text-green-600 font-medium text-sm">+{prod.incoming}</span>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => openRestock(prod.id)}
                          className="px-3 py-1 rounded-lg text-xs font-semibold border border-orange-200 text-[#1a4b97] hover:bg-orange-50 transition-colors">
                          Nhập thêm
                        </button>
                      </td>
                    </tr>
            ...
            <div className="bg-white rounded-2xl w-full max-w-sm">
              ...
              <div className="flex gap-3 p-5 border-t border-[#f0f0f0]">
                ...
                <button onClick={handleRestock} disabled={isRestocking || !selectedProd}
                  ...
                  {isRestocking ? t('saving') : t('confirmStockIn')}
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Toast */}
        {toast && (
          <div className="fixed bottom-6 right-6 z-[100] bg-slate-800 text-white px-5 py-3 rounded-xl shadow-xl text-sm font-semibold">
            {toast}
          </div>
        )}
      </main>
