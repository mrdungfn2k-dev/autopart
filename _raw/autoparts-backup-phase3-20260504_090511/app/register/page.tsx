...
export default function RegisterPage() {
  ...
  const handleRegister = () => {
    if (!form.name || !form.email || !form.password) { setError(lang === "zh" ? "请填写完整信息" : "Vui lòng điền đầy đủ thông tin"); return; }
    if (form.password.length < 6) { setError(lang === "zh" ? "密码必须至少6个字符" : "Mật khẩu phải ít nhất 6 ký tự"); return; }
            ...
            <div className="px-8 py-6">
              ...
              {step === 2 && (
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="col-span-2">
                    <label className="text-sm font-semibold text-[#44494d] mb-1.5 block">{t('passwordLabel')} * ({lang === "zh" ? "至少6个字符" : "ít nhất 6 ký tự"})</label>
                    <div className="relative">
                      <input
                        type={showPass ? "text" : "password"}
                        value={form.password}
                        onChange={e => setForm(p => ({ ...p, password: e.target.value }))}
                        placeholder={lang === "zh" ? "至少6个字符" : "Ít nhất 6 ký tự"}
