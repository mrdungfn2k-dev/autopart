...
function ImageUpload({ preview, onFile }: { preview: string | null; onFile: (url: string) => void }) {
  ...
  return (
    <div>
      ...
      {preview && <button onClick={() => onFile("")} className="mt-1 text-xs text-red-400 hover:text-red-600">✕ Xóa ảnh</button>}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.6)" }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          {/* ── Phase 4.1: Bộ thuộc tính kỹ thuật ── */}
          {attributeSets.length > 0 && (
            <div className="border border-[#f0f0f0] rounded-lg p-3 bg-[#fafafa]">
              ...
              <select value={attributeSetId} onChange={e => { setAttributeSetId(e.target.value); setAttributes({}); }} className="w-full px-3 py-2 border border-[#e5e5e5] rounded text-sm bg-white mb-2">
                <option value="">— Không gắn bộ thuộc tính —</option>
                {attributeSets.map((as: any) => <option key={as.id} value={as.id}>{as.name}</option>)}
              </select>
              {selectedSet && Array.isArray(selectedSet.attributes) && selectedSet.attributes.length > 0 && (
                <div className="space-y-2 mt-2 pt-2 border-t border-[#e5e5e5]">
                  {selectedSet.attributes.map((attr: any) => (
                    <div key={attr.id}>
                      <label className="text-xs text-slate-600 mb-0.5 block">{attr.name}{attr.unit ? ` (${attr.unit})` : ""}</label>
                      {attr.type === "select" ? (
          ...
          {channelsList.length > 0 && (
            <div className="border border-[#f0f0f0] rounded-lg p-3 bg-[#fafafa]">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {channelsList.filter((c: any) => c.active).map((c: any) => (
          ...
          {warehousesList.length > 0 && (
            <div className="border border-[#f0f0f0] rounded-lg p-3 bg-[#fafafa]">
              ...
              <div className="space-y-1.5">
                {warehousesList.filter((w: any) => w.active).map((w: any) => (
        ...
        <div className="flex gap-3 p-5 border-t border-[#f0f0f0] sticky bottom-0 bg-white">
          <button onClick={onClose} className="flex-1 py-2.5 border border-[#e5e5e5] rounded-xl text-sm font-semibold text-slate-600 hover:bg-[#f8f8fa]">{t("cancel")}</button>
          <button onClick={handleSave} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: "var(--ap-primary)" }}>
            {isEdit ? "Lưu thay đổi" : "Thêm phụ tùng"}
          </button>
        </div>
      </div>
    </div>
      ...
      <tr className="border-t border-slate-50 hover:bg-[#f8f8fa] transition-colors">
        <td className="px-4 py-3">
          <div className="flex items-center gap-3" style={{ paddingLeft: depth * 24 + 'px' }}>
            {depth > 0 && <span className="text-slate-300 ml-1">├─</span>}
            ...
            <div>
              <p className="font-semibold text-[#44494d] text-sm flex items-center gap-2">
                {cat.name}
                {hasSubs && (
                  <button onClick={() => setExpanded(!expanded)} className="text-xs text-slate-400 hover:text-slate-600 font-mono">
                    {expanded ? '▼' : '▶'}
                  </button>
                )}
              </p>
        ...
        <td className="px-4 py-3">
          <div className="flex gap-1 flex-wrap w-[160px]">
            <button onClick={() => onEdit(cat, path)}
              className="px-2 py-1 rounded-lg text-xs font-semibold border border-[#e5e5e5] text-slate-600 hover:bg-orange-50 hover:border-orange-300 hover:text-[#1a4b97] transition-colors">
              Sửa
            </button>
            <button onClick={() => onAddSub(currentPath)}
              className="px-2 py-1 rounded-lg text-xs font-semibold border border-[#e5e5e5] text-[#1a4b97] hover:bg-[#1a4b97] hover:text-white transition-colors">
              + Mục con
            </button>
            <button onClick={() => onDelete(cat, path)}
              className="px-2 py-1 rounded-lg text-xs font-semibold border border-red-100 text-red-400 hover:bg-red-50 transition-colors">
              Xóa
            </button>
          </div>
        </td>
...
export default function AdminCatalogPage() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden" style={{ background: "var(--ap-page-bg)" }}>
      <AdminSidebar active="/admin/catalog" />
      <main className="flex-1 overflow-auto">
        {/* Top bar */}
        <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
          <div className="flex items-center justify-between px-6 h-16">
            <h1 className="text-xl font-bold text-[#44494d]">Danh mục &amp; Phụ tùng</h1>
          ...
          <div className="px-6 flex gap-0 border-t border-[#f0f0f0]">
            {[["categories", `Danh mục (${categories.length})`], ["products", `Tất cả phụ tùng (${prods.length})`]].map(([key, label]) => (
        ...
        <div className="p-6">
          ...
          {activeTab === "products" && (
            <div>
              ...
              {/* Filters */}
              <div className="bg-white rounded-xl border border-[#f0f0f0] p-4 mb-4 flex flex-wrap gap-3 items-center">
              ...
              <div className="bg-white rounded-xl border border-[#f0f0f0] overflow-hidden">
                      ...
                      <tr key={prod.id} className="border-t border-slate-50 hover:bg-[#f8f8fa] transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-lg bg-[#f4f4f4] flex items-center justify-center shrink-0 overflow-hidden">
                              {prod.image && prod.image.startsWith("data:")
                                ? <img src={prod.image} alt={prod.name} className="w-full h-full object-cover" />
                                : <span className="text-xs font-bold text-[#8f9294]">{prod.categoryName?.slice(0, 3) || "PT"}</span>}
                            </div>
                            <div>
                        ...
                        <td className="px-4 py-3">
                          <span className="flex items-center gap-1 text-sm font-medium text-yellow-600">
                            * {prod.rating} <span className="text-[#8f9294] text-xs">({prod.reviewCount})</span>
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            <button onClick={() => { setEditingProd(prod); setShowProdModal(true); }}
                              className="px-2 py-1 rounded-lg text-xs font-semibold border border-[#e5e5e5] text-slate-600 hover:bg-orange-50 hover:border-orange-300 hover:text-[#1a4b97]">
                              Sửa
                            </button>
                            <button onClick={() => setDeletingProd(prod)}
                              className="px-2 py-1 rounded-lg text-xs font-semibold border border-red-100 text-red-400 hover:bg-red-50">
                              Xóa
                            </button>
                          </div>
                        </td>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-t border-[#f0f0f0] text-sm text-[#8f9294]">
                  <span>Hiển thị {filtered.length} / {prods.length} phụ tùng</span>
                </div>
              </div>
            </div>
          )}
