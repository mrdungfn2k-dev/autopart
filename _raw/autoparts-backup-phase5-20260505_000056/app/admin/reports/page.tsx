...
allProducts.forEach(p => { _catCount[p.categoryId] = (_catCount[p.categoryId] ?? 0) + p.sold; });
...
const _catColors = ["var(--ap-primary)", "var(--ap-primary)", "#22C55E", "#8B5CF6", "#F59E0B", "#8f9294"];
const categoryData = Object.entries(_catCount).sort((a,b) => b[1] - a[1]).slice(0, 5).map(([id, count], i) => ({
  name: _catLabels[id] ?? id,
 ...
 <AdminSidebar active="/admin/reports" />
 ...
 {[["week", (lang === "zh" ? "周" : "Tuần")], ["month", "Tháng"], ["quarter", "Quý"], ["year", "Năm"]].map(([k, l]) => (
 <button key={k} onClick={() => setPeriod(k)}
 className={`px-3 py-1.5 transition-colors ${period === k ? "text-white font-semibold" : "text-slate-600 hover:bg-[#f8f8fa]"}`}
 style={period === k ? { background: "var(--ap-primary)" } : {}}>
 {l}
 </button>
 ))}
 </div>
 <button className="flex items-center gap-2 px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm text-slate-600 hover:bg-[#f8f8fa]">
 Xuất PDF
 </button>
 </div>
 </div>
 <div className="px-6 flex border-t border-[#f0f0f0]">
 {[["overview", "Tổng quan"], ["products", (lang === "zh" ? "产品" : "Sản phẩm")], ["geography", "Địa lý"]].map(([key, label]) => (
 ...
 <YAxis hide />
 <Tooltip contentStyle={{ borderRadius: "8px", border: "1px solid #E2E8F0" }} formatter={(v) => [`${Number(v)}M`, ""]} />
 <Line type="monotone" dataKey="gmv" stroke="var(--ap-primary)" strokeWidth={3} dot={{ fill: "var(--ap-primary)", r: 5 }} name="GMV" />
 <Line type="monotone" dataKey="orders" stroke="var(--ap-primary)" strokeWidth={2} dot={{ fill: "var(--ap-primary)", r: 4 }} name="Số đơn" />
 </LineChart>
 </ResponsiveContainer>
 ...
 <span className="flex items-center gap-1.5 text-xs text-[#8f9294]"><span className="w-3 h-1 rounded inline-block" style={{ background: "var(--ap-primary)" }}></span>Số đơn hàng</span>
 </div>
 </div>
 ...
 <PieChart>
 <Pie data={categoryData} dataKey="value" cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={3}>
 ...
 <span className={`w-7 h-7 rounded-full text-xs font-extrabold flex items-center justify-center ${i === 0 ? "bg-yellow-100 text-yellow-700" : i === 1 ? "bg-[#f4f4f4] text-slate-600" : i === 2 ? "bg-orange-100 text-orange-700" : "text-[#8f9294]"}`}>
 {i + 1}
 </span>
 </td>
 ...
 <span className="font-bold text-slate-600">{p.revenue}M</span>
 </div>
