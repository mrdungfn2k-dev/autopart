  ...
  return (
    <div>
      ...
      <input id="favicon-upload-input" ref={ref} type="file" accept=".png,.ico,.jpg,.jpeg,.svg" onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }}
        className="block w-full text-sm text-[#44494d] file:mr-3 file:px-3 file:py-1.5 file:rounded file:border-0 file:bg-[#1a4b97] file:text-white file:font-semibold" />
      {preview && (
        <div className="mt-3 flex items-center gap-2">
          <button onClick={handleUpload} disabled={uploading} className="px-4 py-1.5 rounded text-sm font-semibold text-white disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>
            {uploading ? "Đang lưu..." : "Áp dụng favicon"}
          </button>
          {status === "ok" && <span className="text-green-600 text-xs">✓ Đã cập nhật</span>}
...
export default function AdminSettingsPage() {
  ...
  const handleSave = async () => {
    await fetch("/api/settings", {
      ...
      body: JSON.stringify({ branding, commitments, footer, general, features, security, theme, emailTemplates, customCss, customJs, seoMeta, currency }),
    });
    setSaved(true);
  ...
  {!loading && tab === "seo" && (
    <div className={cardCls}>
      <h2 className="font-bold text-[#44494d] mb-2">SEO Meta theo trang</h2>
      ...
      <div className="space-y-4">
        {Object.entries(seoMeta).map(([slug, meta]) => (
