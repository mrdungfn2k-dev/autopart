    ...
    <div className="min-h-screen" style={{ background: "#f8f8fa" }}>
      ...
      <div className="bg-white border-b border-[#e5e5e5] sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex gap-4 text-sm text-[#8f9294]">
            <Link href="/about" className="hover:text-[#44494d]">{lang === "zh" ? "关于我们" : "Về chúng tôi"}</Link>
            <Link href="/login" className="font-semibold px-4 py-1.5 rounded-lg text-white" style={{ background: "var(--ap-primary)" }}>{lang === "zh" ? "登录" : "Đăng nhập"}</Link>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="mt-8 bg-white rounded-2xl border border-[#f0f0f0] p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { title: (lang === "zh" ? "在线聊天" : "Chat trực tiếp"), desc: (lang === "zh" ? "每天 8:00–22:00" : "Hỗ trợ 8h-22h, T2-CN"), action: (lang === "zh" ? "开始聊天" : "Bắt đầu chat"), color: "var(--ap-primary)" },
              { title: "Hotline", desc: "1900 1234 — 24/7", action: (lang === "zh" ? "现在拨打" : "Gọi ngay"), color: "#22C55E" },
              { title: "Email", desc: "support@autopart.vn", action: (lang === "zh" ? "发电子邮件" : "Gửi email"), color: "var(--ap-primary)" },
            ].map(c => (
              <div key={c.title} className="text-center p-5 rounded-2xl border-2 border-[#f0f0f0] hover:border-[#e5e5e5] transition-all">
                ...
                <button className="px-4 py-2 rounded-xl text-sm font-bold text-white" style={{ background: c.color }}>
                  {c.action}
                </button>
              </div>
            ))}
