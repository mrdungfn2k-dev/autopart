    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.65)" }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#8f9294] mb-1">
              ID hãng (không thay đổi sau khi tạo) <span className="text-red-500">*</span>
            </label>
            <input
              value={form.id || ""}
          ...
          <div>
            ...
            <select value={form.region || "japan"} onChange={e => set("region", e.target.value)}
              className="w-full px-3 py-2.5 border border-[#e5e5e5] rounded-lg text-sm bg-white focus:outline-none focus:border-[#1a4b97]">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.65)" }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={key} onChange={e => setKey(e.target.value)} disabled={isEdit} maxLength={3}
              ...
              placeholder="JTD" />
            {isEdit && <p className="text-xs text-amber-600 mt-1">⚠ Mã WMI bị khóa để bảo vệ link tra cứu đã nhúng</p>}
          </div>
          <div>
            <label className="block text-xs font-semibold text-[#8f9294] mb-1">Tên hãng <span className="text-red-500">*</span></label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.type || "Passenger"} onChange={e => set("type", e.target.value)}
                className="w-full px-3 py-2.5 border border-[#e5e5e5] rounded-lg text-sm bg-white focus:outline-none focus:border-[#1a4b97]">
                {["Passenger","SUV","Truck","Pickup","Van","Commercial","Compact","Luxury","Sports","Electric","Minivan","Motorcycle","Performance","Ultra Luxury"].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 border border-[#e5e5e5] rounded-xl text-sm font-semibold text-slate-600 hover:bg-[#f8f8fa]">Hủy</button>
          <button onClick={handleSave} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: "var(--ap-primary)" }}>
            {isEdit ? "Lưu thay đổi" : "Thêm mã WMI"}
          </button>
        </div>
      </div>
    </div>
...
export default function AdminVehiclesPage() {
  ...
  const filteredBrands = brands.filter(b =>
    (!regionFilter || b.region === regionFilter) &&
    (!brandSearch || b.name.toLowerCase().includes(brandSearch.toLowerCase()) || b.id.includes(brandSearch.toLowerCase()))
  );
  const wmiEntries = Object.entries(wmiCodes).filter(([k, v]) =>
    !wmiSearch || k.toLowerCase().includes(wmiSearch.toLowerCase()) || v.brand.toLowerCase().includes(wmiSearch.toLowerCase())
  );
  const filteredVINs = sampleVINs.filter(v =>
    !vinSearch || v.vin.includes(vinSearch.toUpperCase()) || v.brand.toLowerCase().includes(vinSearch.toLowerCase()) || v.model.toLowerCase().includes(vinSearch.toLowerCase())
  );

  return (
    <div className="flex h-screen w-full overflow-hidden" style={{ background: "var(--ap-page-bg)" }}>
      <AdminSidebar active="/admin/vehicles" />
      <main className="flex-1 overflow-auto">
        {/* Top bar */}
        <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
          <div className="flex items-center justify-between px-6 h-16">
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-bold text-[#44494d]">Quản lý Hãng xe & VIN</h1>
              {saving && <span className="text-xs text-[#8f9294] animate-pulse">Đang lưu...</span>}
            </div>
            <div>
              {activeTab === "brands" && (
                    ...
                    {filteredBrands.map(b => (
                      <tr key={b.id} className="border-t border-slate-50 hover:bg-[#f8f8fa] transition-colors">
                        <td className="px-4 py-3">
                          <div className="w-10 h-10 rounded-lg bg-[#f4f4f4] flex items-center justify-center overflow-hidden border border-[#f0f0f0]">
                            {b.logo ? <img src={b.logo} alt={b.name} className="w-full h-full object-contain p-1" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} /> : <span className="text-xs font-bold text-[#8f9294]">{b.id.slice(0,2).toUpperCase()}</span>}
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${REGION_COLORS[b.region] || "bg-[#f4f4f4] text-[#8f9294]"}`}>
                            {REGIONS[b.region] || b.region}
                          </span>
                        </td>
            ...
            <div>
              <div className="flex flex-wrap gap-3 mb-4 items-center">
                <input value={wmiSearch} onChange={e => setWmiSearch(e.target.value)}
                  ...
                  className="px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:outline-none focus:border-[#1a4b97] w-56" />
                {wmiSearch && <button onClick={() => setWmiSearch("")} className="text-xs text-[#8f9294] hover:text-red-500 border border-[#e5e5e5] px-2 py-1.5 rounded-lg">✕ Xóa</button>}
            ...
            <div>
              <div className="flex flex-wrap gap-3 mb-4 items-center">
                <input value={vinSearch} onChange={e => setVinSearch(e.target.value)}
                  ...
                  className="px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:outline-none focus:border-[#1a4b97] w-64" />
                {vinSearch && <button onClick={() => setVinSearch("")} className="text-xs text-[#8f9294] hover:text-red-500 border border-[#e5e5e5] px-2 py-1.5 rounded-lg">✕ Xóa</button>}
