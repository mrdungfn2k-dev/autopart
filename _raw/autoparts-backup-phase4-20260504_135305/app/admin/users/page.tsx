                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={async () => {
                            if (!confirm(`Đăng nhập với tài khoản ${user.email}? Bạn sẽ rời phiên admin hiện tại.`)) return;
                            try {
                              const res = await fetch("/api/admin/impersonate", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: user.email }) });
                              const data = await res.json();
                              if (res.ok) window.location.href = data.redirect || "/";
                              else alert(data.error || "Không thể đăng nhập làm tài khoản này");
                            } catch { alert("Lỗi kết nối"); }
                          }}
                            className="px-2 py-1 rounded-lg text-xs font-bold border border-blue-200 text-blue-600 hover:bg-blue-50" title="Admin: đăng nhập với tài khoản này">Vào tk</button>
