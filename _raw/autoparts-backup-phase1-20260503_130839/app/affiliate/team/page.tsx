...
export default function AffiliateTeamPage() {
 ...
 <AffiliateSidebar active="/affiliate/team" />
 <main className="flex-1 overflow-auto">
 <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
 <div className="flex items-center justify-between px-6 h-16">
 <h1 className="text-xl font-bold text-[#44494d]">Đội nhóm CTV của tôi</h1>
 <button onClick={() => {}} className="flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "var(--ap-primary)" }}>
 + Mời CTV mới
 </button>
 </div>
 </div>
 ...
 { label: "Doanh thu nhóm T10", value: `${(totalTeamCommission / 100000 * 10).toFixed(0)}M`, color: "var(--ap-primary)" },
 { label: "Hoa hồng thụ động (5%)", value: useLang().fp(passiveForMe), color: "var(--ap-primary)" },
 ].map(s => (
 ...
 {/* Referral link */}
 <div className="bg-white rounded-xl border border-[#f0f0f0] p-5 mb-6">
 <h2 className="font-bold text-[#44494d] mb-3">Link giới thiệu CTV mới</h2>
 <div className="flex items-center gap-3">
 <div className="flex-1 px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm font-mono text-[#8f9294] truncate" style={{ background: "#f8f8fa" }}>
 https://autopart.vn/join?ref=LP_GOLD_2024
 </div>
 <button onClick={() => {}} className="px-4 py-2.5 rounded-xl font-semibold text-white text-sm whitespace-nowrap" style={{ background: "var(--ap-primary)" }}>
 Sao chép
 </button>
 <button onClick={() => {}} className="px-4 py-2.5 rounded-xl font-semibold border border-[#e5e5e5] text-slate-600 text-sm whitespace-nowrap hover:bg-[#f8f8fa]">
 Chia sẻ
 </button>
 </div>
 ...
 <div className="flex rounded-lg border border-[#e5e5e5] overflow-hidden bg-white text-sm">
 {[["all", (lang === "zh" ? "全部" : "Tất cả")], ["active", "Đang hoạt động"], ["inactive", "Không hoạt động"]].map(([k, l]) => (
 ...
 style={filterStatus === k ? { background: "var(--ap-primary)" } : {}}>
 {l}
 </button>
 ))}
 </div>
 ...
 <div key={member.id} className="bg-white rounded-xl border border-[#f0f0f0] p-4 flex items-center gap-4">
 ...
 <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${member.status === "active" ? "bg-green-100 text-green-700" : "bg-[#f4f4f4] text-[#8f9294]"}`}>
 {member.status === "active" ? "Đang HĐ" : "Không HĐ"}
 </span>
 </div>
 ...
 <button onClick={() => {}} className="p-2 rounded-lg hover:bg-orange-50 shrink-0">
 </button>
 </div>
 ))}
 </div>
 ...
 <p className="text-xs text-[#1a4b97]">
 Khi CTV trong nhóm bạn (T2) bán được hàng, họ nhận <strong>10%</strong> hoa hồng từ đơn hàng. 
