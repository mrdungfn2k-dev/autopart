...
export default function AffiliateLinksPage() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden" style={{ background: "var(--ap-page-bg)" }}>
      <AffiliateSidebar active="/affiliate/links" />
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6">
          ...
          <div className="space-y-3">
            {links.map(link => (
              <div key={link.id} className="bg-white rounded-xl border border-[#f0f0f0] p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 mr-4">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg max-w-lg" style={{ background: "#f8f8fa", border: "1px solid #E2E8F0" }}>
                      ...
                      <button onClick={() => handleCopy(link.id, link.url)} className="shrink-0 p-1 rounded hover:bg-[#f4f4f4] text-xs text-[#8f9294]">
                        {copied[link.id] ? "✓" : lang === "zh" ? "复制" : "Sao chép"}
                      </button>
                  ...
                  <button onClick={() => handleDelete(link.id)} disabled={deleting === link.id}
                    className="shrink-0 p-1.5 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 transition-colors text-sm">
                    {deleting === link.id ? "..." : "✕"}
                  </button>
                </div>
            ...
            <div className="bg-white rounded-2xl w-full max-w-md">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="p-3 rounded-xl" style={{ background: "#f8f8fa" }}>
                  ...
                  <p className="text-xs font-mono text-slate-600">
                    https://autopart.vn/{newLinkTarget}?ref=LP...&src={newLinkName.replace(/\s/g, "_") || "new"}
                  </p>
                </div>
              </div>
              <div className="flex gap-3 p-5 border-t border-[#f0f0f0]">
                ...
                <button onClick={handleCreate} disabled={creating || !newLinkName.trim()}
                  ...
                  {creating ? "Đang tạo..." : lang === "zh" ? "创建" : "Tạo link"}
                </button>
              </div>
            </div>
          </div>
