...
export default function AffiliateWithdrawPage() {
 ...
 <AffiliateSidebar active="/affiliate/withdraw" />
 ...
 {[["request", "Yêu cầu rút tiền"], ["history", "Lịch sử rút tiền"], ["banks", "Tài khoản ngân hàng"]].map(([key, label]) => (
 ...
 <p className="text-2xl font-bold text-yellow-400">{fp(3200000)}</p>
 <p className="text-xs text-[#8f9294] mt-1">1 yêu cầu đang chờ</p>
 </div>
 <div>
 <p className="text-[#8f9294] text-sm mb-1">Đã rút tổng cộng</p>
 <p className="text-2xl font-bold text-green-400">{fp(27500000)}</p>
 ...
 className="px-3 py-1 border border-[#e5e5e5] rounded-lg text-xs font-semibold text-slate-600 hover:border-[#1a4b97] hover:text-[#1a4b97]">
 {(amt / 1000000).toFixed(0)}M
 </button>
 ))}
 <button onClick={() => setAmount(available.toString())}
 className="px-3 py-1 border border-[#e5e5e5] rounded-lg text-xs font-semibold text-[#1a4b97] border-orange-300">
 Toàn bộ
 </button>
 </div>
 </div>
 ...
 style={selectedBank === bank.id ? { background: "#FFF7ED" } : {}}>
 <input type="radio" name="bank" value={bank.id} checked={selectedBank === bank.id} onChange={() => setSelectedBank(bank.id)} className="accent-orange-500" />
 ...
 {bank.default && <span className="text-xs font-bold px-2 py-0.5 rounded" style={{ background: "#FED7AA", color: "#C2410C" }}>Mặc định</span>}
 </label>
 ))}
 </div>
 </div>
 ...
 <p className="text-xs text-center text-[#8f9294] flex items-center justify-center gap-1">
 ! Xử lý 1-3 ngày làm việc • Chu kỳ giải ngân tự động: mỗi 15 ngày
 </p>
 </div>
 )}
 </div>
 ...
 <span className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${w.status === "COMPLETED" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
 {w.status === "COMPLETED" ? "✓" : ""}
 {w.status === "COMPLETED" ? "Đã chuyển" : (lang === "zh" ? "处理中" : "Đang xử lý")}
 </span>
 </td>
 </tr>
 ))}
 ...
 style={{ background: bank.bank === "Vietcombank" ? "#007B5E" : bank.bank === "Techcombank" ? "#D42F43" : "#AE2070" }}>
 {bank.bank.substring(0, 3).toUpperCase()}
 </div>
 <div className="flex-1">
 ...
 {bank.default && <span className="text-xs font-bold px-2 py-1 rounded" style={{ background: "#FED7AA", color: "#C2410C" }}>Mặc định</span>}
 </div>
 ))}
 <button className="flex items-center gap-2 px-4 py-3 border-2 border-dashed border-slate-300 rounded-xl text-[#8f9294] hover:border-[#1a4b97] hover:text-[#1a4b97] transition-colors w-full justify-center">
