...
export default function ReturnsPage() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden" style={{ background: "#f8f8fa" }}>
      <CustomerSidebar active="/customer/returns" />
        ...
        <div className="p-6 max-w-3xl">
          {submitted && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm font-semibold">
              ✓ {lang === "zh" ? "申请已提交，我们将在1-3个工作日内处理" : "Yêu cầu đã gửi, chúng tôi sẽ xử lý trong 1-3 ngày làm việc"}
            </div>
          )}

          {/* Policy note */}
          <div className="bg-orange-50 border border-orange-100 rounded-xl p-4 mb-5 flex gap-3">
          ...
          {showForm && (
            <div className="bg-white rounded-2xl border border-[#f0f0f0] p-6 mb-5">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-[#44494d] mb-1.5 block">{t("returnOrderId")} *</label>
                  {deliveredOrders.length > 0 ? (
                    <select value={form.orderId} onChange={e => setForm(f => ({ ...f, orderId: e.target.value }))}
                      ...
                      <option value="">{lang === "zh" ? "选择订单" : "Chọn đơn hàng..."}</option>
                ...
                <div>
                  ...
                  <select value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                    className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97] bg-white">
                    <option value="">{lang === "zh" ? "选择原因" : "Chọn lý do..."}</option>
          ...
          {returns.length === 0 && !showForm ? (
            <div className="bg-white rounded-2xl border border-[#f0f0f0] p-14 text-center">
              ...
              <p className="font-semibold text-[#44494d] mb-2">
                {lang === "zh" ? "暂无退换货申请" : "Chưa có yêu cầu đổi trả nào"}
              </p>
            </div>
          ) : (
                  ...
                  <div key={r.id} className="bg-white rounded-2xl border border-[#f0f0f0] p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <span className="text-[#8f9294] text-xs ml-3">
                          {date.getDate()}/{date.getMonth()+1}/{date.getFullYear()}
                        </span>
                      </div>
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${cfg.color}`}>
                        {cfg.icon} {t(`return${r.status.charAt(0).toUpperCase() + r.status.slice(1)}` as any)}
                      </span>
                    </div>
                    <p className="text-sm text-[#8f9294] mb-1">
                      {lang === "zh" ? "订单：" : "Đơn hàng: "}<span className="font-mono text-[#44494d]">{r.orderId}</span>
                    </p>
