    ...
    <section className="bg-white rounded-[12px] px-[16px] py-[12px] shadow-sm mb-[10px] border border-transparent hover:border-[#1a4b97] transition-colors mt-[8px]">
      <div className="flex flex-col xl:flex-row items-start xl:items-center gap-4 justify-between">
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full xl:w-auto">
          ...
          <select value={brand} onChange={e => setBrand(e.target.value)} className="flex-1 sm:w-[140px] bg-[#f8f8fa] text-[#44494d] border border-gray-200 rounded-[6px] px-3 py-1.5 outline-none focus:border-[#1a4b97] text-[13.5px] transition-colors cursor-pointer min-w-0">
            <option value="">{t("selectBrand")}</option>
            {catalogBrands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
          <select value={year} onChange={e => setYear(e.target.value)} className="flex-1 sm:w-[110px] bg-[#f8f8fa] text-[#44494d] border border-gray-200 rounded-[6px] px-3 py-1.5 outline-none focus:border-[#1a4b97] text-[13.5px] transition-colors cursor-pointer min-w-0">
            <option value="">{t("selectYear")}</option>
            {years.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
          <Link
            href={brand ? `/catalog?brand=${encodeURIComponent(brand)}${year ? `&year=${year}` : ""}` : "/catalog"}
            className="px-5 py-1.5 rounded-[6px] font-bold text-[13.5px] text-white transition-colors hover:opacity-90 whitespace-nowrap mt-2 sm:mt-0 w-full sm:w-auto text-center"
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full xl:w-auto mt-2 xl:mt-0">
          ...
          <div className="relative flex-1 sm:w-[200px]">
            <input
              ...
              maxLength={17}
              className="w-full bg-[#f8f8fa] text-[#44494d] border border-gray-200 rounded-[6px] px-3 py-1.5 outline-none focus:border-[#1a4b97] text-[13.5px] tracking-widest uppercase transition-colors"
            />
        ...
        <div className="p-[8px] flex flex-col flex-1 justify-between min-h-[100px] mt-[4px]">
          ...
          <div className="flex flex-col mt-[4px]">
            <div className="flex items-start flex-col mt-0">
               ...
               <span className="text-[14px] text-[#44494d] leading-[19.6px] font-normal tracking-[0.02em]">¥ {prod.cnyPrice ?? (prod.price / 3500).toFixed(1)}</span>
            </div>
  ...
  return (
    <div className="rounded-[12px] p-[16px] relative mb-[40px] shadow-sm" style={{ backgroundImage: 'url(/ap-assets/unmissable-blue.png)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundColor: '#fff' }}>
        <div className="flex items-center justify-between mb-[20px]">
          ...
          <Link href="/products" className="text-[14px] font-medium text-[#3b7cec] cursor-pointer hover:opacity-80">
        ...
        <div className="flex gap-[16px]">
          ...
          <div className="flex-1">
            ...
            <div className="flex justify-end mt-[12px]">
              <Link href="/products" className="text-[14px] font-medium text-[#3b7cec] flex items-center gap-1 hover:opacity-80">
                {lang === "zh" ? "查看更多" : "Xem thêm"}
...
// ─── SẢN PHẨM MỚI (Infinite Scroll ─ giống tính năng load-more) ─────────────
function NewProductsSection() {
  ...
  return (
    <section className="mb-[40px]">
      ...
      <div className="flex items-center justify-between mb-[16px] sticky top-0 z-20 bg-[#f8f8fa] py-[10px]">
        <div className="flex items-center gap-[8px]">
      ...
      {visibleCount < products.length && (
        <div className="flex justify-center mt-[20px]">
          <button
            ...
            className="px-[40px] py-[12px] bg-white border border-[#ddd] rounded-full text-[14px] font-medium text-[#44494d] hover:border-[#1a4b97] hover:text-[#1a4b97] transition-colors shadow-sm"
          >
            {lang === "zh" ? "查看更多产品" : "Xem thêm sản phẩm"}
          </button>
        </div>
      )}
    </section>
        ...
        {suppliers.length === 0
          ...
          <Link href={`/suppliers/${sup.id}`} key={sup.id} className="flex gap-3 items-center group cursor-pointer border-b border-gray-50 pb-3 z-10 relative">
             <div className="w-[45px] h-[45px] rounded-full border border-gray-200 p-0.5 relative shrink-0 ml-2">
                <img loading="lazy" decoding="async" src={sup.logo || "/ap-assets/img-product-clone.png"} className="w-full h-full object-cover rounded-full" onError={(e) => { (e.target as HTMLImageElement).src = "/ap-assets/img-product-clone.png"; }} />
                {renderBadge(idx)}
             </div>
      ...
      <div className="flex items-center gap-2 mt-auto pt-2 border-t border-gray-50 z-10 relative">
          <Link href="/suppliers" className="flex -space-x-2 cursor-pointer hover:-translate-y-1 transition-transform group" title={lang === 'zh' ? '查看全部' : 'Xem tất cả nhà cung cấp'}>
             {suppliers.slice(0, 2).map((sup: any, i: number) => (
                <img key={i} loading="lazy" decoding="async" src={sup?.logo || "/ap-assets/img-product-clone.png"} className="w-[28px] h-[28px] rounded-full border-2 border-white object-cover bg-white" />
             ))}
             {total > 2 && (
...
function SuggestedGrid() {
  ...
  const [prods, setProds] = useState<any[]>([]);
  useEffect(() => {
    fetch("/api/products").then(r=>r.json()).then(d=> setProds(Array.isArray(d)?d.slice(0,12):[])).catch(()=>{});
  }, []);

  if (prods.length === 0) return null;
  
  // Divide into 3 chunks of 4 products (for 3 columns)
  const chunks = [];
  for (let i = 0; i < 3; i++) chunks.push(prods.slice(i*4, (i+1)*4));

  return (
    <div className="flex-1 flex gap-4 min-w-0">
      {chunks.map((chunk, i) => (
        <div key={i} className="flex-1 rounded-[12px] p-3 flex flex-col shadow-sm relative overflow-hidden bg-gradient-to-br from-[#ffffff] to-[#eff4fc] border border-[#e0e7ff]">
          {/* Background Watermark */}
          <svg className="absolute -right-4 -bottom-4 w-32 h-32 text-[#1a4b97] opacity-[0.02] pointer-events-none transform -rotate-12" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" />
          </svg>
          <h3 className="text-[14px] font-bold text-[#44494d] mb-3 ml-1 relative z-10">Có thể bạn đang tìm kiếm</h3>
          <div className="grid grid-cols-2 grid-rows-2 gap-2 flex-1 relative z-10">
            {chunk.map((p, j) => (
              <Link key={j} href={`/products/${p.id}`} className="relative rounded-lg overflow-hidden group cursor-pointer border border-gray-100">
                <img loading="lazy" decoding="async" src={p.image || p.img || p.thumbnail || "/ap-assets/img-product-clone.png"} alt="" className="w-full h-full aspect-square object-cover group-hover:scale-105 transition-transform duration-300" />
                <div className="absolute bottom-1.5 left-1.5 bg-white/95 backdrop-blur px-1.5 py-[1px] rounded-[6px] shadow-sm max-w-[90%] z-10 flex items-center justify-center"><span className="text-[#333] font-bold text-[10px] leading-tight truncate">{fp(p.price || 0).replace(' đ','đ')}</span></div>
              </Link>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-[12px] mb-8 pb-4">
      ...
      <div className="flex gap-[12px] h-[280px]">
        ...
        {/* Middle Suggested Boxes */}
        <div className="hidden xl:flex flex-1 min-w-0 h-full"><SuggestedGrid /></div>
        {/* Right Slider */}
        <div className="w-full lg:w-[330px] xl:w-[360px] shrink-0 h-full">
           <APHeroSlider />
        </div>
      </div>
