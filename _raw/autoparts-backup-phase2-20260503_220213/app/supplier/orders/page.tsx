...
export default function SupplierOrdersPage() {
 ...
 const nextStatusMap: Record<string, string> = {
  ...
  const nextStatusLabel: Record<string, string> = {
    pending: lang === "zh" ? "确认订单" : "Xác nhận đơn", confirmed: "Đã giao cho shipper", shipping: t('confirmDelivered')
  };
 ...
 <SupplierSidebar active="/supplier/orders" />
 ...
 <button className="flex items-center gap-2 px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm text-slate-600 hover:bg-[#f8f8fa] transition-colors">
 {t('exportReport')}
 </button>
 </div>

 {/* Status tabs */}
 <div className="bg-white border-b border-[#f0f0f0] px-6 flex gap-1 overflow-x-auto">
 {statusFilters.map(f => (
 <button key={f.key} onClick={() => setStatusFilter(f.key)}
 className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${statusFilter === f.key ? "border-[#1a4b97] text-[#1a4b97]" : "border-transparent text-[#8f9294] hover:text-[#44494d]"}`}>
 {f.label} <span className="ml-1 text-xs text-[#8f9294]">({f.count})</span>
 </button>
 ))}
 </div>
 ...
 {[[(lang === "zh" ? "客户名" : "Tên KH"), selected.shipping.name], [(lang === "zh" ? "电话" : "SĐT"), selected.shipping.phone], [(lang === "zh" ? "省/市" : "Tỉnh/TP"), selected.shipping.city], [(lang === "zh" ? "下单日" : "Ngày đặt"), formatDate(selected.createdAt)], [t('trackingNumber'), selected.tracking || (lang === "zh" ? "暂无" : "Chưa có")]].map(([k, v]) => (
 ...
 <button onClick={() => advanceStatus(selected.id)} className="w-full py-3 rounded-xl text-white font-bold mt-2" style={{ background: "var(--ap-primary)" }}>
  {nextStatusLabel[selected.status] || "Cập nhật trạng thái"}
 </button>
 </>
 )}
 </div>
