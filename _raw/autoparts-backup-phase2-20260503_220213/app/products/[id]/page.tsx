...
import { addToCart as addToCartStore, saveSelectedIds, saveSelectedVouchers } from "@/lib/cartStore";
...
export default function ProductDetailPage() {
 ...
 /
 <Link href={`/products?category=${product.categoryId}`} className="hover:text-[#1a4b97]">{product.categoryName}</Link>
 /
 ...
 <div className="bg-white rounded-2xl border border-[#f0f0f0] aspect-[4/3] flex items-center justify-center mb-3 relative">
 <span style={{background: getSwatch(product.image).bg, color: getSwatch(product.image).color, fontSize: "16px", fontWeight: "800", padding: "10px 16px", borderRadius: "8px"}}>{getSwatch(product.image).label}</span>
 {product.discountPct && (
 ...
 {[1, 2, 3, 4].map(i => (
 <div key={i} className={`flex-1 aspect-square rounded-xl flex items-center justify-center border-2 cursor-pointer transition-colors ${i === 1 ? "border-[#1a4b97]" : "border-[#e5e5e5] hover:border-orange-300"}`} style={{ background: "#f8f8fa" }}>
 <span style={{background: getSwatch(product.image).bg, color: getSwatch(product.image).color, fontSize: "16px", fontWeight: "800", padding: "10px 16px", borderRadius: "8px"}}>{getSwatch(product.image).label}</span>
 </div>
 ))}
 </div>
 ...
 <p className="text-[#8f9294] text-sm font-mono mb-3">{t('oemCode')}: <span className="font-bold text-slate-600">{product.oemCode}</span></p>
  ...
  <button className="p-2.5 rounded-xl border border-[#e5e5e5] hover:border-slate-300 transition-colors" title={t('share')}>
 ...
 <button onClick={() => setQty(q => q+1)} className="px-4 py-3 hover:bg-[#f4f4f4] transition-colors font-bold text-slate-600">+</button>
 </div>
 {product.stock > 0 ? (
   ...
   <div className="mt-8 bg-white border border-[#f0f0f0] rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm">
     ...
     <div className="flex items-center gap-4">
       <div className="w-16 h-16 rounded-full border-2 border-white shadow-md overflow-hidden bg-[#f8f8fa] shrink-0">
         <img src={supplier.logo || "/ap-assets/img-product-clone.png"} alt={supplier.name} className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).src = "/ap-assets/img-product-clone.png"; }} />
       </div>
       <div>
         ...
         <div className="flex items-center gap-2">
           <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#eef2ff] hover:bg-[#1a4b97] text-[#1a4b97] hover:text-white transition-colors rounded border border-[#1a4b97] text-xs font-semibold">
 ...
 <div className="aspect-video flex items-center justify-center text-5xl" style={{ background: "#f8f8fa" }}><span style={{background: getSwatch(p.image).bg, color: getSwatch(p.image).color, fontSize: "16px", fontWeight: "800", padding: "10px 16px", borderRadius: "8px"}}>{getSwatch(p.image).label}</span></div>
 <div className="p-3">
