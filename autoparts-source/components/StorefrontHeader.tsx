    ...
    <header className="bg-white w-full shadow-sm sticky top-0 z-50 transition-all">
        ...
        <div className="ap-container flex justify-between items-center py-[8px] min-h-[34px]">
          ...
          <div className="flex items-center gap-[20px]">
            <div className="relative" ref={langMenuRef}>
              ...
              >
                <img src={lang === 'en' ? `${A}/en.svg` : lang === 'zh' ? `${A}/zh.svg` : `${A}/vi.svg`} alt={(lang || 'vi').toUpperCase()} className="w-[20px] h-[14px] object-contain rounded-sm" />
              ...
              {showLangMenu && (
                <div className="absolute right-0 top-full mt-1 w-[130px] bg-white rounded-md shadow-[0_4px_12px_rgba(0,0,0,0.1)] border border-gray-100 z-[60] py-1">
                  ...
                  >
                    <img src={`${A}/en.svg`} alt="EN" className="w-[24px] h-[16px] object-cover rounded shadow-[0_0_2px_rgba(0,0,0,0.2)]" />
                    <span className={`text-[13px] ${lang === 'en' ? 'font-bold text-[#1a4b97]' : 'font-medium text-[#44494d]'}`}>English</span>
                  </div>
                  <div
                    className="flex items-center gap-2 px-3 py-2 hover:bg-[#f8f9fa] cursor-pointer transition-colors"
                  ...
                  >
                    ...
                    <span className={`text-[13px] ${lang === 'zh' ? 'font-bold text-[#1a4b97]' : 'font-medium text-[#44494d]'}`}>简体中文</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="ap-container h-[54px] relative flex items-center justify-between">
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-full max-w-[760px] flex-row items-center gap-[6px] z-0 px-4">
          ...
          <label className="bg-[#e8f0fd] text-[#1a4b97] px-[12px] h-[38px] rounded-full font-medium text-[13px] flex items-center justify-center gap-1.5 hover:bg-[#dbeafe] border border-[#bfdbfe] transition-colors shrink-0 cursor-pointer">
            <img src={`${A}/image-search-blue.svg`} alt="" className="w-[15px] h-[15px]" />
            {lang === "zh" ? "图片搜索" : lang === "en" ? "Search by image" : "Tìm ảnh"}
            <input type="file" accept="image/*" className="hidden" onChange={(e) => {
              const f = e.target.files?.[0];
              if (!f) return;
              if (!f.type.startsWith("image/")) { window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Vui lòng chọn tệp ảnh hợp lệ.", type: "warning" } })); e.target.value = ""; return; }
              window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: lang === "zh" ? "正在按图片识别…" : lang === "en" ? "Recognizing image…" : "Đang nhận diện ảnh…", type: "info" } }));
              const reader = new FileReader();
              reader.onload = async (ev) => {
                const { aHashFromSrc } = await import("@/lib/imageHash");
                const hash = await aHashFromSrc(ev.target?.result as string);
        ...
        <div className="flex items-center shrink-0 z-10 pl-4 gap-[16px]">
          ...
          {user ? (
            ...
            <Link href="/login" className="flex items-center gap-[6px] bg-gradient-to-r from-[#1a4b97] to-[#0d2d5e] text-white px-5 py-[7px] rounded-full shadow-sm hover:shadow-md hover:scale-[1.02] transition-all">
