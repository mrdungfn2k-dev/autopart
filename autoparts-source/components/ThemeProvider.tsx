...
export default function ThemeProvider() {
  useEffect(() => {
    ...
    // Áp dụng NGAY khi admin bấm "Lưu cài đặt" ở trang Cài đặt (không cần tải lại trang)
    const onThemeUpdate = (e: Event) => {
      const theme = (e as CustomEvent).detail as Partial<ThemeColors> | undefined;
      if (theme && style) style.textContent = buildCSS({ ...DEFAULTS, ...theme });
    };
    window.addEventListener("ap-theme-updated", onThemeUpdate as EventListener);

    return () => {
      window.removeEventListener("ap-theme-updated", onThemeUpdate as EventListener);
