...
export default function StorefrontFooter() {
  ...
  const toast = (message: string, type = "info") => window.dispatchEvent(new CustomEvent("app-toast", { detail: { message, type } }));
  const handleSubscribe = () => {
    const email = newsletter.trim();
    if (!isEmail(email)) { toast("Email chưa đúng định dạng (vd: ten@gmail.com)", "warning"); return; }
    // Phải đã đăng nhập / có tài khoản mới nhận được ưu đãi
    if (!getAuth()) { setShowLoginPrompt(true); return; }
    fetch("/api/newsletter", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, source: "footer" }) })
      .then(() => {
        setNewsletterDone(true);
        toast(ft?.newsletterPromo ? `Đăng ký thành công! Ưu đãi của bạn: ${ft.newsletterPromo}` : "Đăng ký nhận ưu đãi thành công!", "success");
      })
    ...
    <footer className="bg-[#17212d] text-[#8391a3] text-[14px] font-sans w-full">
      <div className="ap-container py-[40px] flex flex-col md:flex-row gap-8 justify-between mx-auto max-w-7xl px-6">
        ...
        <div className="flex-1 md:max-w-[300px]">
          ...
          {/* Cụm "Kết nối với..." + icon MXH ở footer đã ẩn — đã có thanh nổi góc phải (SocialBar) */}
        </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <p className="font-semibold text-white text-sm mb-0.5">
                {ft?.newsletterTitle || (lang === "zh" ? "订阅促销信息" : "Đăng ký nhận khuyến mãi")}
              </p>
              <p className="text-[#8391a3] text-xs">
                {ft?.newsletterSubtitle || (lang === "zh" ? "第一时间获取Flash Sale和专属优惠" : "Nhận Flash Sale & ưu đãi độc quyền sớm nhất")}
              </p>
            </div>
            {newsletterDone ? (
      ...
      {showLoginPrompt && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 p-4" onClick={() => setShowLoginPrompt(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold text-[#44494d] text-lg mb-2">Cần tài khoản để nhận ưu đãi</h3>
            <p className="text-sm text-[#607080] mb-5">Vui lòng đăng nhập hoặc đăng ký tài khoản để nhận khuyến mãi này.</p>
            <div className="flex gap-3 justify-center">
              <a href="/login" className="px-5 py-2.5 rounded-xl text-sm font-bold border border-[#e5e5e5] text-[#44494d] hover:bg-[#f8f8fa]">Đăng nhập</a>
              <a href="/register" className="px-5 py-2.5 rounded-xl text-sm font-bold text-white" style={{ background: "var(--ap-primary)" }}>Đăng ký</a>
            </div>
          </div>
        </div>
      )}
