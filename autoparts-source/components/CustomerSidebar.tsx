...
const NAV_ICON: Record<string, string> = {
  ...
  "/customer/tracking":      "M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zm-1.5 9c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z",
...
export default function CustomerSidebar({
 active }: { active?: string }) {
  ...
  const menuItems = [
    ...
    { label: t("trackOrder"),        href: "/customer/tracking" },
  ...
  return (
    <aside className="w-[220px] shrink-0 min-h-screen flex flex-col" style={{ background: "var(--ap-sidebar-bg)" }}>
      ...
      <div className="p-4 border-t border-[#2f3336]">
        <button onClick={() => { try { window.dispatchEvent(new CustomEvent("open-chat")); } catch {} }}
          className="w-full py-2 px-3 rounded-lg text-sm font-semibold text-white flex items-center justify-center gap-1.5 hover:opacity-90 transition-opacity" style={{ background: "var(--ap-primary)" }}>
          💬 {t('contactSupport')}
        </button>
      </div>
      <SidebarControls />
