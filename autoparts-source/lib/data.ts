...
// Định dạng tiền VND gọn & ĐỒNG NHẤT: Tỷ / Triệu / đ (không dùng "M")
export function formatVndShort(vnd: number): string {
  const n = Math.abs(vnd || 0);
  if (n >= 1e9) { const v = vnd / 1e9; return (Number.isInteger(v) ? v : +v.toFixed(2)).toLocaleString("vi-VN") + " Tỷ"; }
  if (n >= 1e6) { const v = vnd / 1e6; return (Number.isInteger(v) ? v : +v.toFixed(1)).toLocaleString("vi-VN") + " Triệu"; }
