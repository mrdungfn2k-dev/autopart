...
export default function CustomerReviewsPage() {
  ...
  const pagedReviews = reviews.slice((reviewPageSafe - 1) * REVIEWS_PER_PAGE, reviewPageSafe * REVIEWS_PER_PAGE);
        ...
        <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
          <div className="flex items-center justify-between px-6 h-14">
            ...
            <button onClick={() => setShowForm(true)}
              ...
              style={{ background: "var(--ap-primary)" }}>+ {t("writeReview")}
            </button>
          </div>
        </div>

        <div className="p-6">{submitted && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm font-semibold">✓ {lang === "zh" ? "评价已提交，审核通过后将公开显示" : "Đánh giá đã gửi, sẽ hiển thị sau khi được duyệt"}
            </div>)}
          ...
          {showForm && (
            <div className="ap-card bg-white rounded-2xl border border-[#f0f0f0] p-6 mb-5">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-[#44494d] mb-1.5 block">{lang === "zh" ? "选择产品 *" : "Sản phẩm đã mua *"}
                  </label>{purchasedProducts.length > 0 ? (
                    <select value={form.productId}
                      ...
                      <option value="">{lang === "zh" ? "选择产品..." : "Chọn sản phẩm..."}</option>{purchasedProducts.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>) : (
                ...
                <div>
                  <label className="text-sm font-semibold text-[#44494d] mb-2 block">{lang === "zh" ? "评分 *" : "Đánh giá sao *"}
                  </label>
              ...
              <div className="flex gap-3 mt-5">
                <button onClick={handleSubmit}
                  ...
                  style={{ background: "var(--ap-primary)" }}>{t("submitReview")}
                </button>
          ...
          {reviews.length === 0 && !showForm ? (
            <div className="ap-card bg-white rounded-2xl border border-[#f0f0f0] p-14 text-center">
              <p className="font-semibold text-[#44494d] mb-2">{lang === "zh" ? "您还没有提交过评价" : "Bạn chưa có đánh giá nào"}
              </p>
              <p className="text-sm text-[#8f9294]">{lang === "zh" ? "购买产品后分享您的使用体验" : "Sau khi mua hàng, hãy chia sẻ trải nghiệm của bạn"}
              </p>
            </div>) : (
            <div className="grid md:grid-cols-2 gap-3">{pagedReviews.map(r => {
                ...
                return (
                  <div key={r.id} className="ap-card bg-white rounded-2xl border border-[#f0f0f0] p-5">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${cfg.color}`}>{t(`review${r.status.charAt(0).toUpperCase() + r.status.slice(1)}` as any)}
                      </span>
                    </div>{r.title && <p className="font-semibold text-[#44494d] text-sm mb-1">{r.title}</p>}
                    <p className="text-sm text-[#44494d] leading-relaxed">{r.content}</p>
                  </div>);
              })}
            </div>)}
          {reviews.length > 0 && <Pagination page={reviewPageSafe} totalPages={reviewTotalPages} onChange={setReviewPage} totalItems={reviews.length} pageSize={REVIEWS_PER_PAGE} unit={lang === "zh" ? "条评价" : lang === "en" ? "reviews" : "đánh giá"} />}
        </div>
      </main>
    </>);
}
