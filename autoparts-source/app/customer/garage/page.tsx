...
export default function CustomerGaragePage() {
  ...
  const addVehicle = async () => {
    ...
    if (!form.brand || !form.model.trim() || !plate) {
      window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Vui lòng chọn Hãng xe và nhập Dòng xe, Biển số xe.", type: "warning" } }));
      return;
    }
    if (form.model.trim().length < 2 || !/[A-Za-zÀ-ỹ0-9]/.test(form.model)) {
      window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Dòng xe không hợp lệ (VD: Vios, CR-V...).", type: "warning" } }));
      return;
    }
    // Biển số VN: 2 số + 1-2 chữ + 3-5 số (+ .XX) — vd 51A-123.45, 30F-12345, 29A12345
    if (!/^\d{2}[A-Z]{1,2}[\s-]?\d{3,5}(\.\d{2})?$/.test(plate)) {
      window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Biển số xe không hợp lệ (VD: 51A-123.45 hoặc 30F-12345).", type: "warning" } }));
      return;
    }
    setSaving(true);
    try {
      ...
      <main className="flex-1 overflow-auto">
        <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
          <div className="flex items-center justify-between px-6 h-14">
            <h1 className="text-lg font-bold text-[#44494d]">{lang === "zh" ? "我的车库" : "Gara của tôi"} ({garage.length} {lang === "zh" ? "辆" : "xe"})
            </h1>
            <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "var(--ap-primary)" }}>+ {lang === "zh" ? "添加车辆" : "Thêm xe"}
            </button>
          </div>
        </div>

        <div className="p-6">{loading ? (
            <div className="flex items-center justify-center h-64 text-[#8f9294]">{lang === "zh" ? "加载中..." : "Đang tải..."}
            </div>) : (
            <>
              <div className="grid md:grid-cols-2 gap-5 mb-6">{garage.map((vehicle) => (
                  <div key={vehicle.id} className="ap-card bg-white rounded-2xl border border-[#f0f0f0] overflow-hidden">{/* Vehicle header */}
                    <div className="p-5 pb-4" style={{ background: "linear-gradient(135deg, #0F172A, #1E293B)" }}>
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-1">
                          <button onClick={() => setEditingVehicle(vehicle)} className="px-2 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-white text-xs font-semibold">{lang === "zh" ? "编辑" : "Sửa"}
                          </button>
                          <button onClick={() => deleteVehicle(vehicle.id)} className="px-2 py-1 rounded-lg bg-white/10 hover:bg-[#1a4b97]/30 transition-colors text-red-300 text-xs font-semibold">{lang === "zh" ? "删除" : "Xóa"}
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="p-5 pt-4">
                      ...
                      <div className="flex gap-2">
                        <Link href={`/products?q=${vehicle.brand}+${vehicle.model}+${vehicle.year}`}
                          ...
                          style={{ background: "var(--ap-primary)" }}>{lang === "zh" ? "适配配件" : "Phụ tùng phù hợp"}
                        </Link>
                        <Link href="/customer/orders" className="px-4 py-2.5 rounded-xl border border-[#e5e5e5] text-slate-600 text-sm hover:bg-[#f8f8fa]">{lang === "zh" ? "历史" : "Lịch sử"}
                        </Link>
                      </div>
                    </div>
                  </div>))}
              ...
              </div>{/* Service reminder */}
              {garage.length > 0 && (
                <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
                  <h2 className="font-bold text-[#44494d] mb-3">{lang === "zh" ? "保养提醒" : "Nhắc lịch bảo dưỡng"}</h2>
                  <div className="space-y-3">{garage.map((v) => (
                      <div key={v.id} className="flex items-center gap-4 p-3 rounded-xl border border-[#f0f0f0]">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-[#8f9294]">{lang === "zh" ? "下次保养：" : "Bảo dưỡng định kỳ tiếp theo: "}
                            <span className="font-bold text-[#1a4b97]">{v.nextService}</span>
                          </p>
                        </div>
                        <Link href="/products" className="px-3 py-1.5 rounded-lg text-xs font-bold text-white" style={{ background: "var(--ap-primary)" }}>{lang === "zh" ? "订购配件" : "Đặt phụ tùng"}
                        </Link>
                      </div>))}
                  </div>
                </div>)}
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-md">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select value={form.brand} onChange={(e) => setForm((p) => ({ ...p, brand: e.target.value }))} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm">
                      <option value="">{lang === "zh" ? "选择品牌" : "Chọn hãng"}</option>{brandOptions.map((b) => <option key={b}>{b}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-slate-600 mb-1 block">{t("vehicleModel")}</label>
                    <input value={form.model} maxLength={40} onChange={(e) => setForm((p) => ({ ...p, model: e.target.value }))} placeholder="VD: Vios, City..." className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-slate-600 mb-1 block">{t("vehicleYear")}</label>
                    <select value={form.year} onChange={(e) => setForm((p) => ({ ...p, year: e.target.value }))} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm">{Array.from({ length: 20 }, (_, i) => 2025 - i).map((y) => <option key={y}>{y}</option>)}
                    </select>
                  </div>
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <input value={form.licensePlate} maxLength={12} onChange={(e) => setForm((p) => ({ ...p, licensePlate: e.target.value.toUpperCase().replace(/[^0-9A-Z.\- ]/g, "") }))} placeholder="VD: 51A-123.45" className={`w-full px-4 py-2.5 border rounded-xl text-sm font-mono focus:outline-none focus:border-[#1a4b97] ${form.licensePlate && !/^\d{2}[A-Z]{1,2}[\s-]?\d{3,5}(\.\d{2})?$/.test(form.licensePlate.trim()) ? "border-red-400" : "border-[#e5e5e5]"}`} />
                  </div>
                </div>
              </div>
              <div className="flex gap-3 p-5 border-t border-[#e5e5e5]">
                ...
                <button onClick={addVehicle} disabled={saving || !form.brand || !form.model} className="flex-1 py-2.5 rounded-xl font-semibold text-white disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>{saving ? "..." : t("addVehicle")}
                </button>
              </div>
            </div>
          </div>)}
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-md">
              ...
              <div className="flex gap-3 p-5 border-t border-[#e5e5e5]">
                ...
                <button onClick={saveEdit} disabled={saving} className="flex-1 py-2.5 rounded-xl font-semibold text-white disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>{saving ? "..." : (lang === "zh" ? "保存" : "Lưu")}
                </button>
              </div>
            </div>
          </div>)}
