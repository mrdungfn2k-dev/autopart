...
const statusSymbol: Record<string, { symbol: string; bg: string; color: string }> = {
 ...
 refunded: { symbol: "₫", bg: "#F5F3FF", color: "#8B5CF6" },
};
...
export default function CustomerOrdersPage() {
 const { t, fp, lang } = useLang();
  ...
  const statusTabs = [
   ...
   { key: "refunded", label: (lang === "zh" ? "已退款" : "Đã hoàn tiền") },
  ];
 ...
 function handleReorder(order: ApiOrder) {
   ...
   for (const it of order.items) {
     addToCart(it.id, it.qty, { name: it.name, price: it.price });
   }
   setReorderMsg(`Đã thêm ${order.items.length} sản phẩm từ đơn ${order.id} vào giỏ hàng.`);
   setTimeout(() => router.push("/cart"), 800);
 }

 async function handleConfirmReceived(order: ApiOrder) {
   try {
     ...
     if (res.ok) {
       ...
       setReorderMsg(`Đã xác nhận nhận hàng cho đơn ${order.id}.`);
 ...
 <Link href="/products" className="flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "var(--ap-primary)" }}>Mua thêm
 </Link>
 </div>{/* Status tabs */}
 ...
 {loading ? (
   <div className="text-center py-16 text-[#8f9294]">
     <span className="inline-block w-6 h-6 border-2 border-[#1a4b97] border-t-transparent rounded-full animate-spin mr-2" />Đang tải đơn hàng...
 ...
 <div className="space-y-3">{paged.map(order => {
 ...
 <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 font-bold text-base" style={{ background: sym.bg, color: sym.color }}>{sym.symbol}
 </div>
 <div>
 ...
 <div className="border-t border-[#f0f0f0] p-4" style={{ background: "#FAFAFA" }}>{/* Shipping progress */}
 ...
 </div>{order.tracking && <p className="text-xs text-[#1a4b97] font-mono">Mã tracking: {order.tracking}</p>}
 </div>)}

 {/* Order items */}
 <div className="mb-4">
   <p className="text-xs text-[#8f9294] mb-2 font-semibold">{lang === "zh" ? "产品" : "SẢN PHẨM"}</p>{order.items.map((item, i) => (
 ...
 <div className="flex gap-3 flex-wrap">{order.status === "delivered" && (
 <>
 <button onClick={() => handleReorder(order)} className="px-4 py-2 rounded-lg text-sm font-semibold text-white hover:opacity-90 transition-opacity" style={{ background: "var(--ap-primary)" }}>Mua lại</button>
 <Link href="/customer/reviews" className="px-4 py-2 rounded-lg text-sm font-semibold border border-[#e5e5e5] text-[#44494d] hover:bg-[#f8f8fa]">Đánh giá</Link>
 </>)}
 {order.status === "shipping" && (
 <button onClick={() => handleConfirmReceived(order)} className="px-4 py-2 rounded-lg text-sm font-semibold text-white hover:opacity-90 transition-opacity" style={{ background: "#22C55E" }}>Đã nhận hàng</button>)}
 <Link href={`/customer/tracking?id=${order.id}`} className="px-4 py-2 rounded-lg text-sm font-semibold border border-[#e5e5e5] text-[#44494d] hover:bg-[#f8f8fa]">{order.status === "shipping" ? "Theo dõi đơn" : "Xem chi tiết"}
 </Link>
 </div>
 </div>)}
 </div>);
 ...
 <Pagination page={pageSafe} totalPages={totalPages} onChange={setPage} totalItems={filtered.length} pageSize={PAGE_SIZE} unit={lang === "zh" ? "笔订单" : lang === "en" ? "orders" : "đơn"} />
 </div>
 </main>{reorderMsg && (
   <div className="fixed bottom-6 right-6 bg-green-600 text-white px-5 py-3 rounded-xl shadow-lg text-sm font-semibold z-50">{reorderMsg}
   </div>)}
 </>);
}
