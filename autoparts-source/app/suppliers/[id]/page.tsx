...
export default function SupplierDetailPage() {
  ...
  // Phân trang sản phẩm trong shop (tránh cuộn dài)
  const shopTotalPages = Math.max(1, Math.ceil(filteredProducts.length / PER_PAGE));
  const shopPageSafe = Math.min(shopPage, shopTotalPages);
  const pagedProducts = filteredProducts.slice((shopPageSafe - 1) * PER_PAGE, shopPageSafe * PER_PAGE);
  useEffect(() => { setShopPage(1); }, [search, selectedCategory]);
        ...
        <div className="bg-white rounded-2xl border border-[#f0f0f0] shadow-sm px-5 py-4 -mt-5 relative z-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="w-[72px] h-[72px] rounded-2xl border-4 border-white shadow-lg overflow-hidden bg-white -mt-10 shrink-0">
              <img
                src={supplier.logo || "/ap-assets/supplier-default.svg"}
                alt={supplier.name}
        ...
        {activeTab === "products" && (
          <div className="flex flex-col md:flex-row gap-6 mb-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Phân trang sản phẩm shop */}
              {shopTotalPages > 1 && (
                <div className="flex items-center justify-center gap-1 mt-6">
                  <button onClick={() => setShopPage(p => Math.max(1, p - 1))} disabled={shopPageSafe <= 1}
                    className="w-9 h-9 rounded-lg flex items-center justify-center border border-[#e5e5e5] text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#f4f4f4] text-base">‹</button>
                  {Array.from({ length: shopTotalPages }, (_, i) => i + 1)
                    .filter(n => n === 1 || n === shopTotalPages || Math.abs(n - shopPageSafe) <= 1)
                    .reduce<(number | string)[]>((acc, n, i, arr) => {
                      if (i > 0 && n - (arr[i - 1] as number) > 1) acc.push("…");
                      acc.push(n); return acc;
                    }, [])
                    .map((n, i) => n === "…" ? (
                      <span key={`e${i}`} className="w-9 h-9 flex items-center justify-center text-slate-400 text-xs">…</span>
                    ) : (
                      <button key={n} onClick={() => setShopPage(n as number)}
                        className={`w-9 h-9 rounded-lg text-sm font-semibold transition-colors ${n === shopPageSafe ? "text-white" : "border border-[#e5e5e5] text-slate-600 hover:bg-[#f4f4f4]"}`}
                        style={n === shopPageSafe ? { background: "var(--ap-primary)" } : {}}>{n}</button>
                    ))}
                  <button onClick={() => setShopPage(p => Math.min(shopTotalPages, p + 1))} disabled={shopPageSafe >= shopTotalPages}
                    className="w-9 h-9 rounded-lg flex items-center justify-center border border-[#e5e5e5] text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#f4f4f4] text-base">›</button>
                </div>
              )}
            </div>
