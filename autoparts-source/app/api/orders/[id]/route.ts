...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Staff (admin/supplier) may edit any field. A customer may only confirm
  // receipt of their own order (shipping → delivered).
  const isStaff = user.role === "admin" || user.role === "supplier";
  const isOwner = orders[idx].userId === user.id;
  if (!isStaff && !(isOwner && body.status === "delivered")) {
