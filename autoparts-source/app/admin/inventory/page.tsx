    ...
    <>
      <main className="flex-1 overflow-auto">{/* Top bar */}
        ...
        <div className="p-6">{saved && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm font-semibold">✓ {lang === "zh" ? "库存已更新" : "Đã cập nhật tồn kho"}
            </div>)}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">{[
              ...
              <div key={i} className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-4">
                <p className="text-xs text-[#8f9294] font-semibold uppercase mb-2">{kpi.label}</p>
                <p className="text-2xl font-bold" style={{ color: kpi.color }}>{kpi.value}</p>
              </div>))}
          </div>

          <div className="grid lg:grid-cols-[1fr_320px] gap-6">{/* Product inventory table */}
            <div className="ap-card bg-white rounded-2xl border border-[#f0f0f0] overflow-hidden">
              <div className="p-4 border-b border-[#f0f0f0] flex flex-wrap gap-3 items-center">
                ...
                <div className="flex gap-2">{(["all", "low", "out"] as const).map(f => (
                    <button key={f} onClick={() => setFilter(f)}
                      ...
                      style={filter === f ? { background: "var(--ap-primary)" } : {}}>{f === "all" ? (lang === "zh" ? "全部" : "Tất cả") : f === "low" ? (lang === "zh" ? "低库存" : "Sắp hết") : (lang === "zh" ? "缺货" : "Hết hàng")}
                    </button>))}
                </div>
              </div>
              <div className="overflow-x-auto">{loading ? (
                  <div className="p-12 text-center text-[#8f9294]">
                    <span className="inline-block w-6 h-6 border-2 border-[#1a4b97] border-t-transparent rounded-full animate-spin mr-2" />{lang === "zh" ? "加载中..." : "Đang tải..."}
                  </div>) : (
                  <table className="w-full text-sm">
                    ...
                    <tbody>{pg.paged.map(p => (
                        <tr key={p.id} className="border-t border-[#f8f8fa] hover:bg-[#f8f8fa]/60">
                          ...
                          <td className="px-4 py-3">
                            <span className={`font-bold text-sm px-2 py-0.5 rounded-full ${stockColor(p.stock)}`}>{p.stock}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`text-xs font-semibold ${p.stock === 0 ? "text-red-600" : p.stock <= LOW_STOCK_THRESHOLD ? "text-orange-600" : "text-green-700"}`}>{p.stock === 0 ? (lang === "zh" ? "缺货" : "Hết hàng") : p.stock <= LOW_STOCK_THRESHOLD ? (lang === "zh" ? "低库存" : "Sắp hết") : (lang === "zh" ? "正常" : "Ổn định")}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <button onClick={() => { setAdjustTarget(p); setAdjustDelta(""); setAdjustNote(""); }}
                              className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white"
                              style={{ background: "var(--ap-primary)" }}>{t("stockAdjust")}
                            </button>
                          </td>
                        </tr>))}
                      {filtered.length === 0 && (
            ...
            <div className="space-y-4">{/* Adjust form */}
              {adjustTarget && (
                <div className="bg-white rounded-2xl border-2 p-5" style={{ borderColor: "var(--ap-primary)" }}>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs font-semibold text-[#44494d] mb-1 block">{lang === "zh" ? "调整数量（+入库 / -出库）" : "Điều chỉnh (+nhập / -xuất) *"}
                      </label>
                    ...
                    <div>
                      ...
                      <input value={adjustNote} onChange={e => setAdjustNote(e.target.value)}
                        ...
                        className="w-full px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm focus:outline-none focus:border-[#1a4b97]" />
                    </div>{adjustDelta && !isNaN(parseInt(adjustDelta)) && (
                      <div className="text-xs text-[#44494d] p-2 bg-[#f0f6ff] rounded-lg">{lang === "zh" ? "调整后库存：" : "Tồn sau điều chỉnh: "}<strong>{Math.max(0, adjustTarget.stock + parseInt(adjustDelta))}</strong>
                      </div>)}
                  </div>
                  <div className="flex gap-2 mt-4">
                    <button onClick={handleAdjust}
                      className="flex-1 py-2 rounded-lg text-sm font-bold text-white"
                      style={{ background: "var(--ap-primary)" }}>{t("confirm")}
                    </button>
                    <button onClick={() => setAdjustTarget(null)}
                      className="px-4 py-2 rounded-lg text-sm font-bold border border-[#e5e5e5] text-[#44494d]">{t("cancel")}
                    </button>
                  </div>
                </div>)}
              ...
              <div className="ap-card bg-white rounded-2xl border border-[#f0f0f0] p-5">
                <h3 className="font-bold text-[#44494d] mb-4">{t("stockHistory")}</h3>{history.length === 0 ? (
                      ...
                      return (
                        <div key={i} className="flex items-start justify-between py-2 border-b border-[#f8f8fa] last:border-0">
                          ...
                          <div className="text-right shrink-0 ml-2">
                            <span className={`text-xs font-bold ${h.delta > 0 ? "text-green-600" : "text-red-600"}`}>{h.delta > 0 ? "+" : ""}{h.delta}
                            </span>
