      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6">{/* KPI */}
          ...
          <div className="space-y-3">{loading ? (
              <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-10 text-center text-[#8f9294]">{lang === "zh" ? "加载中..." : "Đang tải..."}
              </div>) : filtered.length === 0 ? (
              <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-10 text-center text-[#8f9294]">{lang === "zh" ? "没有需要处理的请求。" : "Không có yêu cầu nào cần xử lý."}
              </div>) : (
              filtered.map((item) => {
                ...
                return (
                  <div key={item.id} className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5 flex items-center gap-4 flex-wrap sm:flex-nowrap">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold shrink-0" style={{ background: "#f8f8fa", color: "var(--ap-primary)" }}>{cfg.abbr}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[#8f9294] mb-1">{lang === "zh" ? "提交者：" : "Gửi bởi: "}
                        <span className="font-semibold text-slate-600">{item.submittedBy}</span> • {item.date}
                      </p>
                      <p className="text-sm text-[#8f9294] truncate">{item.details}</p>{item.province && <p className="text-xs text-[#8f9294] mt-0.5">{item.province}</p>}
                      {item.taxCode && <p className="text-xs text-[#8f9294]">{lang === "zh" ? "税号：" : "MST: "}{item.taxCode}</p>}
                    </div>
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => setSelected(item)} className="px-3 py-1.5 rounded-lg border border-[#e5e5e5] text-xs font-medium text-[#8f9294] hover:bg-[#f8f8fa] whitespace-nowrap">{lang === "zh" ? "详情" : "Chi tiết"}
                      </button>{item.status === "PENDING" && (
                        <>
                          <button onClick={() => approve(item.id)} disabled={isLoading} className="flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: "#22C55E" }}>✓ {lang === "zh" ? "批准" : "Duyệt"}
                          </button>
                          <button onClick={() => reject(item.id)} disabled={isLoading} className="flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: "#EF4444" }}>✕ {lang === "zh" ? "拒绝" : "Từ chối"}
                          </button>
                        </>)}
                    </div>
                  </div>);
            ...
            <div className="w-96 bg-white shadow-xl overflow-y-auto p-6">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "#f8f8fa" }}>
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold" style={{ background: "#FEF3C7", color: "var(--ap-primary)" }}>{typeConfig[selected.type].abbr}
                  </div>
                  <div>
              ...
              </div>{selected.status === "PENDING" && (
                <div className="flex gap-3 mt-6">
                  <button onClick={() => approve(selected.id)} disabled={actionLoading === selected.id} className="flex-1 py-2.5 rounded-xl font-bold text-white flex items-center justify-center gap-1 disabled:opacity-50" style={{ background: "#22C55E" }}>✓ {lang === "zh" ? "批准" : "Phê duyệt"}
                  </button>
                  <button onClick={() => reject(selected.id)} disabled={actionLoading === selected.id} className="flex-1 py-2.5 rounded-xl font-bold text-white flex items-center justify-center gap-1 disabled:opacity-50" style={{ background: "#EF4444" }}>✕ {lang === "zh" ? "拒绝" : "Từ chối"}
                  </button>
                </div>)}
            </div>
          </div>)}
