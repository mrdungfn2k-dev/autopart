...
const categoryData = Object.entries(_catCount).sort((a,b) => b[1] - a[1]).slice(0, 5).map(([id, count], i) => ({
 ...
 { label: (lang === "en" ? "Number of Orders" : lang === "zh" ? "订单数" : "Số đơn hàng"), value: "1,540", prev: "1,190", change: "+29.4%", up: true, color: "#22C55E" },
 { label: "Người dùng mới", value: "221", prev: "172", change: "+28.5%", up: true, color: "var(--ap-primary)" },
 { label: (lang === "en" ? "Return Rate" : lang === "zh" ? "退货率" : "Tỷ lệ hoàn đơn"), value: "2.1%", prev: "2.8%", change: "-0.7%", up: true, color: "#8B5CF6" },
 ].map(s => (
 ...
 <Line type="monotone" dataKey="orders" stroke="#22C55E" strokeWidth={2} dot={{ fill: "#22C55E", r: 4 }} connectNulls name={lang === "en" ? "Orders" : lang === "zh" ? "订单数" : "Số đơn"} />
 </LineChart>
 </ResponsiveContainer>
 ...
 <span className="flex items-center gap-1.5 text-xs text-[#8f9294]"><span className="w-3 h-1 rounded inline-block" style={{ background: "#22C55E" }}></span>{lang === "en" ? "Number of Orders" : lang === "zh" ? "订单数" : "Số đơn hàng"}</span>
 </div>
 </div>
 ...
 <Bar dataKey="orders" fill="var(--ap-primary)" radius={[0,6,6,0]} name={lang === "en" ? "Orders" : lang === "zh" ? "订单数" : "Số đơn"} />
 </BarChart>
 </ResponsiveContainer>
 </div>
