...
import { validateImageFile, imageError, IMG_ACCEPT } from "@/lib/imageValidate";
...
function ImageUpload({ preview, onFile }: { preview: string | null; onFile: (url: string) => void }) {
  ...
  function handle(file: File) {
    const err = validateImageFile(file); if (err) { imageError(err); return; }
...
const bannersApi = {
  ...
  create: async (data: Partial<Banner>) => { const r = await fetch("/api/banners", { method:"POST", credentials:"include", headers:{"Content-Type":"application/json"}, body: JSON.stringify(data) }); if (!r.ok) throw new Error("create-failed"); return r.json(); },
...
function VouchersTab() {
  ...
  const [form, setForm] = useState({ code: "", type: "percent", value: "", minOrder: "", limit: "100", expiry: "", minQty: "0", applicableCategoryIds: [] as string[], applicableProductIds: [] as string[] });
  const [editingId, setEditingId] = useState<string | null>(null);
  const [voucherCats, setVoucherCats] = useState<any[]>([]);
  useEffect(() => {
    fetch("/api/categories").then(r => r.json()).then(d => setVoucherCats(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
  ...
  const remove = async (id: string) => {
    if (!(await confirmDialog("Xóa voucher này?"))) return;
    await vouchersApi.remove(id); load();
  };
  const resetForm = () => setForm({ code: "", type: "percent", value: "", minOrder: "", limit: "100", expiry: "", minQty: "0", applicableCategoryIds: [], applicableProductIds: [] });
  const openCreate = () => { setEditingId(null); resetForm(); setErr(""); setShowModal(true); };
  const openEdit = (v: Voucher) => {
    setForm({ code: v.code, type: v.type, value: String(v.value), minOrder: String(v.minOrder ?? ""), limit: String(v.limit ?? "100"), expiry: v.expiry || "", minQty: String((v as any).minQty ?? 0), applicableCategoryIds: (v as any).applicableCategoryIds ?? [], applicableProductIds: (v as any).applicableProductIds ?? [] });
    setEditingId(v.id); setErr(""); setShowModal(true);
  };
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, type: form.type as "percent" | "fixed" | "shipping", value: Number(form.value), minOrder: Number(form.minOrder), limit: Number(form.limit), minQty: Number(form.minQty) || 0 } as any;
      if (editingId) await vouchersApi.update(editingId, payload);
  ...
  return (
    <div>
      <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-4 mb-4 flex gap-3 items-center">
        ...
        <button onClick={openCreate}
          className="ml-auto px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "var(--ap-primary)" }}>+ Tạo voucher
        </button>
      </div>
      <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] overflow-hidden">
        <table className="w-full">
          <thead style={{ background: "#f8f8fa" }}>
            <tr className="text-xs text-[#8f9294] font-semibold">{["MÃ VOUCHER","LOẠI & GIÁ TRỊ","ĐƠN TỐI THIỂU","ĐÃ DÙNG / GIỚI HẠN","HẾT HẠN","TRẠNG THÁI","HÀNH ĐỘNG"].map(h => (
                <th key={h} className="text-left px-4 py-3">{h}</th>))}
            </tr>
          </thead>
          <tbody>{loading && <tr><td colSpan={7} className="text-center py-8 text-[#8f9294] text-sm">{t("loading")}</td></tr>}
            ...
            {filtered.map(v => (
              <tr key={v.id} className="border-t border-slate-50 hover:bg-[#f8f8fa] transition-colors">
                ...
                <td className="px-4 py-3 font-bold">{v.type === "percent" ? <span className="text-[#1a4b97]">{v.value}%</span>: v.type === "fixed" ? <span className="text-[#1a4b97]">{fp(v.value)}</span>: <span className="text-green-600">{"Miễn ship"}</span>}
                </td>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => toggle(v)} className="text-xs font-semibold">{v.active ? <span className="text-green-600">● Bật</span> : <span className="text-[#8f9294]">○ Tắt</span>}
                  </button>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(v)} className="px-2 py-1.5 rounded-lg hover:bg-blue-50 text-[#1a4b97] text-xs font-bold" title={lang === "zh" ? "编辑" : "Sửa"}>✎ {lang === "zh" ? "编辑" : "Sửa"}</button>
                    <button onClick={() => remove(v.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 text-xs font-bold" title={lang === "zh" ? "删除" : "Xóa"}>✕</button>
                  </div>
                </td>
              </tr>))}
          </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-[#f0f0f0]">
              <h3 className="font-bold text-[#44494d]">{editingId ? (lang === "zh" ? "编辑优惠券" : "Sửa voucher") : (lang === "zh" ? "新建优惠券" : "Tạo mã voucher mới")}</h3>
              <button onClick={closeModal} className="text-[#8f9294] hover:text-slate-600">✕</button>
            </div>
            <div className="p-5 space-y-4">{err && <p className="text-red-500 text-sm bg-red-50 px-3 py-2 rounded-lg">{err}</p>}
              <div>
                <label className="text-sm font-semibold text-slate-600 mb-1 block">Mã Voucher *</label>
                <input value={form.code} disabled={!!editingId} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))}
                  placeholder="VD: SUMMER30" className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm font-mono uppercase focus:outline-none focus:border-[#1a4b97] disabled:bg-gray-50 disabled:opacity-70" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-sm font-semibold text-slate-600 mb-1 block">{"Số lượng SP tối thiểu"}</label>
                  <input type="number" min="0" value={form.minQty} onChange={e => setForm(f => ({ ...f, minQty: e.target.value }))}
                    placeholder="0 = không giới hạn" className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]" />
                </div>
              </div>{/* Phase 4.1: Quy tắc danh mục — chỉ áp dụng cho danh mục cụ thể */}
              <div className="border border-[#e5e5e5] rounded-xl p-3 bg-[#fafafa]">
                <label className="text-sm font-semibold text-slate-600 mb-2 block">Áp dụng cho danh mục <span className="text-xs font-normal text-[#8f9294]">(bỏ trống = tất cả)</span></label>
                <div className="grid grid-cols-2 gap-1.5 max-h-32 overflow-y-auto">{voucherCats.map((c: any) => (
                    <label key={c.id} className="flex items-center gap-2 cursor-pointer text-xs">
                      <input type="checkbox" checked={form.applicableCategoryIds.includes(c.id)} onChange={e => {
                        setForm(f => ({ ...f, applicableCategoryIds: e.target.checked ? [...f.applicableCategoryIds, c.id] : f.applicableCategoryIds.filter(x => x !== c.id) }));
            ...
            <div className="flex gap-3 p-5 border-t border-[#f0f0f0]">
              <button onClick={closeModal} className="flex-1 py-2.5 border border-[#e5e5e5] rounded-xl font-semibold text-slate-600 hover:bg-[#f8f8fa]">{t("cancel")}</button>
              <button onClick={submit} disabled={saving} className="flex-1 py-2.5 rounded-xl font-semibold text-white disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>{saving ? "Đang lưu..." : editingId ? (lang === "zh" ? "保存更改" : "Lưu thay đổi") : (lang === "zh" ? "创建" : "Tạo voucher")}
              </button>
            </div>
          </div>
        </div>)}
...
// ─── Đổi điểm (Rewards) Tab — admin cấu hình quà đổi điểm ─────────────────────
function RewardsTab() {
  ...
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState("");
  const blank = { name: "", nameZh: "", pointsCost: "500", voucherType: "fixed", voucherValue: "50000", minOrder: "0", active: true };
  const [form, setForm] = useState(blank);
  const load = useCallback(() => { setLoading(true); fetch("/api/rewards").then(r => r.json()).then(d => setRewards(Array.isArray(d) ? d : [])).finally(() => setLoading(false)); }, []);
  useEffect(() => { load(); }, [load]);
  const openCreate = () => { setEditingId(null); setForm(blank); setErr(""); setShowModal(true); };
  const openEdit = (r: any) => { setForm({ name: r.name, nameZh: r.nameZh || "", pointsCost: String(r.pointsCost), voucherType: r.voucherType, voucherValue: String(r.voucherValue), minOrder: String(r.minOrder), active: r.active !== false }); setEditingId(r.id); setErr(""); setShowModal(true); };
  const close = () => { setShowModal(false); setEditingId(null); setForm(blank); };
  const toggle = async (r: any) => { await fetch("/api/rewards", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: r.id, active: !r.active }) }); load(); };
  const remove = async (id: string) => { if (!(await confirmDialog(lang === "zh" ? "删除该奖励？" : "Xóa phần quà này?"))) return; await fetch(`/api/rewards?id=${id}`, { method: "DELETE" }); load(); };
  const submit = async () => {
    setErr("");
    if (!form.name || !(Number(form.pointsCost) > 0)) { setErr(lang === "zh" ? "请填写名称和积分" : "Nhập tên quà và số điểm"); return; }
    setSaving(true);
    try {
      const payload: any = { name: form.name, nameZh: form.nameZh, pointsCost: Number(form.pointsCost), voucherType: form.voucherType, voucherValue: Number(form.voucherValue) || 0, minOrder: Number(form.minOrder) || 0, active: form.active };
      const res = await fetch("/api/rewards", { method: editingId ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editingId ? { ...payload, id: editingId } : payload) });
      if (!res.ok) { const d = await res.json().catch(() => ({})); setErr(d?.error || "Lỗi"); return; }
      close(); load();
    } finally { setSaving(false); }
  };
  const valLabel = (r: any) => r.voucherType === "percent" ? `${r.voucherValue}%` : r.voucherType === "shipping" ? (lang === "zh" ? "免运费" : "Miễn ship") : fp(r.voucherValue);

  return (
    <div>
      <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-4 mb-4 flex items-center">
        <p className="text-sm text-[#8f9294]">{lang === "zh" ? "配置客户用积分兑换的奖励" : "Cấu hình các phần quà khách đổi bằng điểm"}</p>
        <button onClick={openCreate} className="ml-auto px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "var(--ap-primary)" }}>+ {lang === "zh" ? "新建奖励" : "Thêm quà"}</button>
      </div>
      <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] overflow-hidden">
        <table className="w-full">
          <thead style={{ background: "#f8f8fa" }}><tr className="text-xs text-[#8f9294] font-semibold">
            {[lang === "zh" ? "奖励" : "PHẦN QUÀ", lang === "zh" ? "积分" : "ĐIỂM", lang === "zh" ? "价值" : "GIÁ TRỊ", lang === "zh" ? "最低订单" : "ĐƠN TỐI THIỂU", lang === "zh" ? "状态" : "TRẠNG THÁI", lang === "zh" ? "操作" : "HÀNH ĐỘNG"].map(h => <th key={h} className="text-left px-4 py-3">{h}</th>)}
          </tr></thead>
          <tbody>{loading && <tr><td colSpan={6} className="text-center py-8 text-[#8f9294] text-sm">...</td></tr>}
            {!loading && rewards.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-[#8f9294] text-sm">{lang === "zh" ? "暂无奖励" : "Chưa có phần quà"}</td></tr>}
            {rewards.map(r => (
              <tr key={r.id} className="border-t border-slate-50 hover:bg-[#f8f8fa]">
                <td className="px-4 py-3 font-semibold text-[#44494d]">{lang === "zh" ? (r.nameZh || r.name) : r.name}</td>
                <td className="px-4 py-3 font-bold" style={{ color: "var(--ap-primary)" }}>{r.pointsCost}</td>
                <td className="px-4 py-3 text-[#44494d] text-sm">{valLabel(r)}</td>
                <td className="px-4 py-3 text-[#8f9294] text-sm">{fp(r.minOrder)}</td>
                <td className="px-4 py-3"><button onClick={() => toggle(r)} className="text-xs font-semibold">{r.active !== false ? <span className="text-green-600">● {lang === "zh" ? "开" : "Bật"}</span> : <span className="text-[#8f9294]">○ {lang === "zh" ? "关" : "Tắt"}</span>}</button></td>
                <td className="px-4 py-3"><div className="flex items-center gap-1">
                  <button onClick={() => openEdit(r)} className="px-2 py-1.5 rounded-lg hover:bg-blue-50 text-[#1a4b97] text-xs font-bold">✎ {lang === "zh" ? "编辑" : "Sửa"}</button>
                  <button onClick={() => remove(r.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 text-xs font-bold">✕</button>
                </div></td>
              </tr>))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-[#f0f0f0]">
              <h3 className="font-bold text-[#44494d]">{editingId ? (lang === "zh" ? "编辑奖励" : "Sửa phần quà") : (lang === "zh" ? "新建奖励" : "Thêm phần quà")}</h3>
              <button onClick={close} className="text-[#8f9294] hover:text-slate-600">✕</button>
            </div>
            <div className="p-5 space-y-4">{err && <p className="text-red-500 text-sm bg-red-50 px-3 py-2 rounded-lg">{err}</p>}
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-sm font-semibold text-slate-600 mb-1 block">{lang === "zh" ? "名称(越南语)" : "Tên (VI)"} *</label>
                  <input value={form.name} maxLength={60} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="VD: Giảm 50,000đ" className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]" /></div>
                <div><label className="text-sm font-semibold text-slate-600 mb-1 block">{lang === "zh" ? "名称(中文)" : "Tên (ZH)"}</label>
                  <input value={form.nameZh} maxLength={60} onChange={e => setForm(f => ({ ...f, nameZh: e.target.value }))} placeholder="减 50,000đ" className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]" /></div>
                <div><label className="text-sm font-semibold text-slate-600 mb-1 block">{lang === "zh" ? "所需积分" : "Điểm cần"} *</label>
                  <input type="number" min={1} value={form.pointsCost} onChange={e => setForm(f => ({ ...f, pointsCost: e.target.value }))} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]" /></div>
                <div><label className="text-sm font-semibold text-slate-600 mb-1 block">{lang === "zh" ? "优惠类型" : "Loại ưu đãi"}</label>
                  <select value={form.voucherType} onChange={e => setForm(f => ({ ...f, voucherType: e.target.value }))} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm bg-white">
                    <option value="fixed">{lang === "zh" ? "减现金(đ)" : "Giảm tiền (đ)"}</option>
                    <option value="percent">{lang === "zh" ? "百分比(%)" : "Phần trăm (%)"}</option>
                    <option value="shipping">{lang === "zh" ? "免运费" : "Miễn phí ship"}</option>
                  </select></div>
                <div><label className="text-sm font-semibold text-slate-600 mb-1 block">{lang === "zh" ? "价值" : "Giá trị"}</label>
                  <input type="number" min={0} value={form.voucherValue} disabled={form.voucherType === "shipping"} onChange={e => setForm(f => ({ ...f, voucherValue: e.target.value }))} placeholder={form.voucherType === "percent" ? "20" : "50000"} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97] disabled:bg-gray-50" /></div>
                <div><label className="text-sm font-semibold text-slate-600 mb-1 block">{lang === "zh" ? "最低订单(đ)" : "Đơn tối thiểu (đ)"}</label>
                  <input type="number" min={0} value={form.minOrder} onChange={e => setForm(f => ({ ...f, minOrder: e.target.value }))} className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]" /></div>
              </div>
              <label className="flex items-center gap-2 text-sm text-[#44494d] cursor-pointer"><input type="checkbox" checked={form.active} onChange={e => setForm(f => ({ ...f, active: e.target.checked }))} className="accent-[#1a4b97]" /> {lang === "zh" ? "启用(显示给客户)" : "Bật (hiển thị cho khách)"}</label>
            </div>
            <div className="flex gap-3 p-5 border-t border-[#f0f0f0]">
              <button onClick={close} className="flex-1 py-2.5 border border-[#e5e5e5] rounded-xl font-semibold text-slate-600 hover:bg-[#f8f8fa]">{lang === "zh" ? "取消" : "Hủy"}</button>
              <button onClick={submit} disabled={saving} className="flex-1 py-2.5 rounded-xl font-semibold text-white disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>{saving ? "..." : editingId ? (lang === "zh" ? "保存" : "Lưu") : (lang === "zh" ? "创建" : "Tạo")}</button>
            </div>
          </div>
        </div>)}
    </div>);
...
function FlashSaleTab() {
  ...
  const remove = async (id: string) => {
    if (!(await confirmDialog("Xóa Flash Sale này?"))) return;
  ...
  const now = new Date();
  return (
    <div className="space-y-4">{!showForm && (
        <button onClick={() => setShowForm(true)}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-white font-semibold text-sm" style={{ background: "var(--ap-primary)" }}>+ Tạo Flash Sale mới
        </button>)}

      {showForm && (
        <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-6">
          ...
          <div className="mb-4">
            ...
            <div className="border border-[#e5e5e5] rounded-xl p-3 max-h-48 overflow-y-auto space-y-1">{allProducts.slice(0, 20).map(p => (
          ...
          <div className="flex gap-3">
            ...
            <button onClick={create} disabled={saving} className="px-6 py-2.5 rounded-xl text-white text-sm font-bold disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>{saving ? "Đang lưu..." : "Lên lịch Flash Sale"}
            </button>
          </div>
        </div>)}

      {loading ? (
        <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-8 text-center text-[#8f9294]">{t("loading")}</div>) : campaigns.length === 0 ? (
        <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-8 text-center text-[#8f9294]">Chưa có Flash Sale nào. Tạo chiến dịch đầu tiên!</div>) : campaigns.map(fs => {
        ...
        const prods = Array.isArray(fs.products) ? fs.products : []; // chống crash khi flash sale thiếu mảng products
        return (
          <div key={fs.id} className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="font-bold text-[#44494d]">{fs.name}</h3>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${isActive ? "bg-green-100 text-green-700" : isScheduled ? "bg-blue-100 text-blue-700" : "bg-[#f4f4f4] text-[#8f9294]"}`}>{isActive ? "Đang chạy" : isScheduled ? "Lên lịch" : fs.active ? "Tắt" : "Đã kết thúc"}
                  </span>
                </div>
                <p className="text-sm text-[#8f9294]">Giảm {fs.discount}% • {prods.length} sản phẩm •{" "}
                  {new Date(fs.startTime).toLocaleString("vi-VN")} → {new Date(fs.endTime).toLocaleString("vi-VN")}
                </p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setExpanded(expanded === fs.id ? null : fs.id)} className="px-3 py-1.5 rounded-lg text-xs font-bold border border-[#1a4b97] text-[#1a4b97] hover:bg-[#eef2ff]">{expanded === fs.id ? "Ẩn" : "Chi tiết"}
                </button>
                <button onClick={() => toggle(fs)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${fs.active ? "border-[#e5e5e5] text-slate-600" : "border-orange-300 text-[#1a4b97]"}`}>{fs.active ? "Tắt" : "Bật"}
                </button>
            ...
            {expanded === fs.id && (
              <div className="mt-4 pt-4 border-t border-[#f0f0f0]">
                <p className="text-xs font-semibold text-[#8f9294] uppercase mb-2">Sản phẩm trong Flash Sale ({prods.length}) · giảm {fs.discount}%</p>
                {prods.length === 0 ? <p className="text-sm text-[#8f9294]">Chưa có sản phẩm nào.</p> : (
                  <div className="space-y-0">{prods.map((p: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 text-sm py-2 border-b border-slate-50 last:border-0">
                      <span className="text-[#44494d] truncate flex-1">{p.name}</span>
                      <span className="text-[#8f9294] line-through text-xs">{(p.originalPrice || 0).toLocaleString("vi-VN")}đ</span>
                      <span className="font-bold text-[#1a4b97] w-24 text-right">{(p.price || 0).toLocaleString("vi-VN")}đ</span>
                      <span className="text-xs font-bold text-red-500 w-12 text-right">-{p.discount}%</span>
                    </div>))}
                  </div>
                )}
              </div>
...
function BannersTab() {
  ...
  const remove = async (id: string) => {
    if (!(await confirmDialog("Xóa banner này?"))) return;
    await bannersApi.remove(id); load();
  };
  const create = async () => {
    if (!form.title.trim()) { window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Vui lòng nhập tiêu đề banner", type: "warning" } })); return; }
    setSaving(true);
    try {
      await bannersApi.create({ ...form, startDate: form.startDate || new Date().toISOString().split("T")[0], endDate: form.endDate || "2030-12-31", status: "active" });
      setShowForm(false);
      setForm({ title: "", subtitle: "", cta: "Khám phá", href: "/products", startDate: "", endDate: "", image: "" });
      load();
      window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Đã thêm banner mới", type: "success" } }));
    } catch {
      window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Không thêm được banner. Vui lòng thử lại hoặc đăng nhập lại.", type: "warning" } }));
    } finally { setSaving(false); }
  };

  return (
    <div className="space-y-4">{!showForm && (
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-white font-semibold text-sm" style={{ background: "var(--ap-primary)" }}>+ Thêm banner mới
        </button>)}
      ...
      {showForm && (
        <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowForm(false)} className="px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm font-semibold text-slate-600">{t("cancel")}</button>
            <button onClick={create} disabled={saving} className="px-6 py-2.5 rounded-xl text-white text-sm font-bold disabled:opacity-50" style={{ background: "var(--ap-primary)" }}>{saving ? "Đang lưu..." : "Tạo banner"}
            </button>
          </div>
        </div>)}

      {loading ? (
        <div className="text-center py-8 text-[#8f9294]">{t("loading")}</div>) : banners.map(b => (
        <div key={b.id} className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5 flex items-center gap-5">{b.image ? (
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${statusColor[b.status]}`}>{b.status === "active" ? "Đang chạy" : b.status === "scheduled" ? "Lên lịch" : "Đã kết thúc"}
              </span>
            </div>
          ...
          <div className="flex gap-2">
            <button onClick={() => toggleStatus(b)} className="px-3 py-1.5 rounded-lg text-xs font-bold border border-[#e5e5e5] text-slate-600 hover:bg-[#f8f8fa]">{b.status === "active" ? "Tắt" : "Bật"}
            </button>
...
function OriginsTab() {
  ...
  const remove = async (id: string) => { if (await confirmDialog("Xóa quốc gia này?")) { await fetch(`/api/origins/${id}`, { method: "DELETE" }); load(); } };

  return (
    <div className="space-y-4">{!showForm && <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-[#0d2d5e] text-white rounded-xl text-sm font-bold">+ Thêm gốc (Quốc gia)</button>}
      {showForm && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">{origins.map(o => (
          <div key={o.id} className="bg-white p-4 rounded-xl border flex items-center justify-between">
            <div className="flex items-center gap-3">{o.image ? (
...
function HomeSectionsTab() {
  ...
  const remove = async (id: string) => { if (await confirmDialog("Xóa section trang chủ này?")) { await fetch(`/api/home-sections/${id}`, { method: "DELETE" }); load(); } };

  return (
    <div className="space-y-4">{!showForm && <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-[#0d2d5e] text-white rounded-xl text-sm font-bold">+ Thêm Section Trang Chủ</button>}
      {showForm && (
        <div className="bg-white p-5 rounded-xl border border-[#f0f0f0]">
      ...
      <div className="flex flex-col gap-3 mt-4">{sections.map(s => (
          <div key={s.id} className="bg-white p-4 rounded-xl border flex items-center justify-between">
            <div className="flex items-center gap-3">{s.icon ? <img src={s.icon} className="w-8 h-8" alt=""/> : <div className="w-8 h-8 bg-slate-100 rounded-full"/>}
              <div>
...
export default function AdminMarketingPage() {
  ...
  const [activeTab, setActiveTab] = useState<"vouchers" | "rewards" | "banners" | "flash_sale" | "origins" | "home_sections">("vouchers");
    ...
    <>
      <main className="flex-1 overflow-auto">
        <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
          ...
          <div className="px-6 flex border-t border-[#f0f0f0] overflow-x-auto">{([ ["vouchers","Mã khuyến mãi"], ["rewards", lang === "zh" ? "积分兑换" : "Đổi điểm"], ["banners","Banner & Quảng cáo"], ["flash_sale","Flash Sale"], ["origins","Khám phá (Quốc gia)"], ["home_sections","Cấu trúc Trang chủ"] ] as const).map(([key, label]) => (
        ...
        <div className="p-6">{activeTab === "vouchers" && <VouchersTab />}
