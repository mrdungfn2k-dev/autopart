...
export default function AdminNewsletterPage() {
  ...
  async function handleDeleteSub(email: string) {
    if (!(await confirmDialog(`Xoá email ${email} khỏi danh sách?`))) return;
  ...
  async function handleDeleteCamp(id: string) {
    if (!(await confirmDialog("Xoá chiến dịch này?"))) return;
  ...
  return (
    <>
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6">{loading ? <div className="text-center py-16 text-[#8f9294]">Đang tải...</div> : (
            <>{tab === "subs" && (
                <>
              ...
              {tab === "campaigns" && (
                camps.length === 0 ? (
                  ...
                  <div className="space-y-3">{camps.map(c => (
                      <div key={c.id} className="ap-card bg-white rounded-2xl border border-[#f0f0f0] p-4">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <div className="flex gap-2">{c.status === "draft" && <button onClick={() => handleSend(c.id)} className="px-3 py-1 rounded text-xs font-bold text-white" style={{ background: "#22C55E" }}>Gửi</button>}
