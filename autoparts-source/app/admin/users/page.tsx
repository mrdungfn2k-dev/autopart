...
export default function AdminUsersPage() {
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
          ...
          {isEdit && (
            <div className="text-[11px] text-[#8f9294] bg-[#f8f8fa] rounded-lg px-3 py-2 mb-4">
              ℹ️ Admin chỉ được sửa <b>Vai trò</b> và <b>Trạng thái</b>. Thông tin cá nhân (tên, email, SĐT) do người dùng tự quản lý.
            </div>
          )}
          {(() => {
          ...
          <div className="space-y-4">{/* Name */}
            ...
            <div>
              ...
              <select value={form.status || "active"} onChange={e => set("status", e.target.value)} disabled={isAdminAcct}
                ...
                {/* "Chờ duyệt" chỉ còn chọn được khi tài khoản đang ở trạng thái chờ duyệt (đã duyệt thì không quay lại) */}
  ...
  // Nguồn THẬT: tài khoản đăng ký từ users.json (/api/admin/users) + doanh nghiệp NCC từ suppliers.json.
  // → tài khoản mới đăng ký hiển thị ngay; bỏ danh sách seed cứng.
  useEffect(() => {
    Promise.all([
      ...
      setUsers(prev => {
        ...
        for (const u of userRecords) byId.set(u.id, u);   // tài khoản thật (users.json)
        for (const s of supRecords) byId.set(s.id, s);     // doanh nghiệp NCC (suppliers.json)
        // giữ user admin tự thêm trong phiên (id u<timestamp>) chưa có trong API
  ...
  // Lưu trạng thái THẬT vào users.json (hoặc suppliers.json với NCC). Trả về true nếu thành công.
  ...
  const handleSaveUser = async (u: UserRecord) => {
    ...
    if (existing) {
      // Sửa: chỉ lưu vai trò + trạng thái (thông tin cá nhân khóa cứng)
      if (isSupplierId(u.id)) {
  ...
  // Khóa: chặn admin + xác nhận + LƯU THẬT
  ...
  // Mở khóa / Duyệt: chuyển sang hoạt động + LƯU THẬT
  ...
  // Xóa: chặn admin + xác nhận + LƯU THẬT
          ...
          <div className="flex items-center justify-between px-6 h-16">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setShowRoleModal(true)}
                className="flex items-center gap-2 px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm text-slate-600 hover:bg-[#f8f8fa]">Quản lý vai trò
              </button>
              <button onClick={openAdd}
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "var(--ap-primary)" }}>+ Thêm người dùng
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-b-xl border border-[#f0f0f0] border-t-0">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(user)}
                            className="px-2 py-1 rounded-lg text-xs font-semibold border border-[#e5e5e5] text-slate-600 hover:bg-[#f4f4f4]">Sửa
                          ...
                          {user.status === "active" && user.role !== "admin" && (
                            <button onClick={() => askLock(user)}
                              className="px-2 py-1 rounded-lg text-xs font-bold border border-red-200 text-red-500 hover:bg-red-50">Khóa</button>)}
                          {user.status === "suspended" && (
                            <button onClick={() => setActive(user, `Đã mở khóa "${user.name}"`)}
                              className="px-2 py-1 rounded-lg text-xs font-bold border border-green-200 text-green-500 hover:bg-green-50">Mở khóa</button>)}
                          {user.role !== "admin" && (
            ...
            <div className="relative flex items-center justify-center px-4 py-3 border-t border-[#f0f0f0]">
              <span className="absolute left-4 text-sm text-[#8f9294] hidden md:block">
                {filtered.length === 0 ? "0" : `${(pageSafe - 1) * PAGE_SIZE + 1}–${Math.min(pageSafe * PAGE_SIZE, filtered.length)}`} / {filtered.length} người dùng
              </span>
              <div className="flex items-center gap-1">
