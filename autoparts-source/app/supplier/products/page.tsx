  ...
  return (
    <div ref={ref} className="relative">
      ...
      >{selected?.icon && <img src={selected.icon} alt="" className="w-5 h-5 object-contain shrink-0" />}
        <span className={selected ? "text-[#44494d]" : "text-[#8f9294]"}>{selected ? selected.label : placeholder}
        </span>
        <svg className="w-3.5 h-3.5 ml-auto text-[#8f9294] shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          <path d="M6 9l6 6 6-6"/>
        </svg>
      </div>{open && !disabled && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-[#e5e5e5] rounded-xl shadow-xl max-h-56 overflow-hidden" style={{ minWidth: "100%" }}>{/* Search input */}
          ...
          <div className="overflow-auto max-h-44">{filtered.length === 0 ? (
              ...
              filtered.map((opt, i) => (
                ...
                >{opt.icon && <img src={opt.icon} alt="" className="w-5 h-5 object-contain shrink-0 rounded" />}
                  <span className="truncate">{opt.label}</span>{opt.value === value && (
    ...
    <div ref={ref} className="relative">
      ...
      {showSuggestions && suggestions.length > 0 && !isValid && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-[#e5e5e5] rounded-xl shadow-xl max-h-52 overflow-hidden">
          ...
          <div className="overflow-auto max-h-44">{suggestions.slice(0, 10).map((s, i) => {
      ...
      {decoded && vin.length >= 3 && (
        <div className="mt-1.5 flex items-center gap-2 px-2.5 py-1.5 bg-[#f0f9f0] border border-green-200 rounded-lg">{brandLogo && <img src={brandLogo} alt="" className="w-5 h-5 object-contain" />}
          <span className="text-xs font-semibold text-green-800">{decoded.brand}</span>
          <span className="text-[10px] text-green-600">{getFlag(decoded.country)} {decoded.country}</span>{isValid && <span className="text-[10px] text-green-500 ml-auto">✓ VIN hợp lệ</span>}
        </div>)}
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ borderColor: drag ? "var(--ap-primary)" : "#CBD5E1", background: drag ? "#EFF4FB" : "#f8f8fa", minHeight: "150px" }}
      >{preview ? (
          <img src={preview} alt="preview" className="w-full h-40 object-contain" />) : (
          <div className="flex flex-col items-center gap-2 py-8">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "#EFF4FB" }}>
...
function ProductFormModal({
  ...
  const handleSave = async () => {
    if (!form.name.trim()) { window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: t('productNameLabel'), type: "warning" } })); return; }
    const priceNum = Number(form.price);
    if (!form.price || priceNum <= 0) { window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: t('priceLabel'), type: "warning" } })); return; }
    if (priceNum > 100000000) { window.dispatchEvent(new CustomEvent("app-toast", { detail: { message: "Giá tối đa 100 triệu đ.", type: "warning" } })); return; }
    setSaving(true);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[92vh] overflow-auto shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                max={100000000}
                value={form.price}
                onChange={e => set("price", e.target.value === "" ? "" : String(Math.min(Math.max(Number(e.target.value) || 0, 0), 100000000)))}
                placeholder="VD: 1250000 (tối đa 100 triệu)"
                className="w-full px-4 py-2.5 border border-[#e5e5e5] rounded-xl text-sm focus:outline-none focus:border-[#1a4b97]"
              />
            </div>
          </div>
          ...
          <div className="border border-[#e0e7f1] rounded-xl p-3 bg-[#f8faff]">
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              <div>
                ...
                >
                  <option value="">-- Từ năm --</option>{Array.from({ length: 36 }, (_, i) => 2025 - i).map(y => (
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-[#e5e5e5] rounded-lg text-sm bg-white focus:outline-none focus:border-[#1a4b97]"
                >
                  <option value="">-- Đến năm --</option>{Array.from({ length: 36 }, (_, i) => 2025 - i).map(y => (
                    <option key={y} value={String(y)}>{y}</option>))}
                </select>
              </div>
            </div>{(form as any).yearFrom && (form as any).yearTo && (
              <p className="text-xs text-[#1a4b97] mt-1.5 font-medium">Tương thích xe đời {(form as any).yearFrom} – {(form as any).yearTo}
              </p>)}
          </div>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.isTrending}
                  ...
                  className="accent-[#1a4b97] w-4 h-4" />
        ...
        <div className="flex gap-3 p-5 border-t border-[#f0f0f0] sticky bottom-0 bg-white">
          <button onClick={onClose} className="flex-1 py-2.5 border border-[#e5e5e5] rounded-xl font-semibold text-slate-600 hover:bg-[#f8f8fa] text-sm">{t("cancel")}
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
...
export default function SupplierProductsPage() {
  ...
  // Mở thẳng form Sửa khi vào từ trang Tồn kho (/supplier/products?edit=<id>)
  useEffect(() => {
    if (typeof window === "undefined" || products.length === 0) return;
    const id = new URLSearchParams(window.location.search).get("edit");
    if (!id) return;
    const p = products.find(x => x.id === id);
    if (p) { setEditingProd(p); setShowModal(true); }
  }, [products]);

  const filtered = myProducts.filter(p =>search === "" ||
    ...
    <>
      <main className="flex-1 overflow-auto">{/* Top bar */}
        ...
        <div className="p-6">{/* Stats */}
          <div className="grid grid-cols-4 gap-4 mb-6">{stats.map(({ label, value, color }) => (
              <div key={label} className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-4">
                <p className="text-[#8f9294] text-xs mb-1">{label}</p>
                <p className="text-2xl font-bold" style={{ color }}>{value}</p>
              </div>))}
          </div>{/* Filters */}
          <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-4 mb-4 flex items-center gap-3 flex-wrap">
            <input
              value={search}
          ...
          <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] overflow-x-auto">{loading ? (
              <div className="text-center py-16 text-[#8f9294]">{t('loading')}</div>) : filtered.length === 0 ? (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 overflow-hidden"
                            style={{ background: getSwatch(prod.image).bg }}>{prod.image?.startsWith("data:")
                              ? <img src={prod.image} alt={prod.name} className="w-full h-full object-cover" />: <span style={{ color: getSwatch(prod.image).color, fontSize: "10px", fontWeight: "bold" }}>{prod.categoryName?.slice(0, 3) || "PT"}
                                </span>}
                          </div>
                          <div>
                            <p className="font-semibold text-[#44494d] text-sm whitespace-nowrap">{lang === "zh" ? ((prod as any).nameZh || prod.name) : prod.name}</p>
                            <p className="text-xs text-[#8f9294] whitespace-nowrap">{lang === "zh" ? ((prod as any).categoryNameZh || prod.categoryName) : prod.categoryName}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 font-mono text-[#8f9294] text-xs whitespace-nowrap">{prod.oemCode}</td>
                      <td className="px-4 py-3">
