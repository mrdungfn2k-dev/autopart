...
export default function SupplierAnalyticsPage() {
  ...
  const _clProd: Record<string, string> = {}; // tên danh mục thật theo dữ liệu SP (tránh hiện slug thô)
  products.forEach((p: any) => { _sc[p.categoryId] = (_sc[p.categoryId] ?? 0) + p.sold; if (p.categoryId && (p.categoryName || p.categoryNameZh)) _clProd[p.categoryId] = lang === "zh" ? (p.categoryNameZh || p.categoryName) : (p.categoryName || p.categoryNameZh); });
      ...
      <main className="flex-1 overflow-auto">
        <div className="sticky top-0 bg-white border-b border-[#e5e5e5] z-10">
          <div className="flex items-center justify-between px-6 h-16">
            ...
            <div className="flex gap-2">{["7 ngày", "30 ngày", "3 tháng", "1 năm"].map(p => (
        ...
        <div className="p-6 space-y-6">{/* KPIs */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[
              ...
              <div key={k.label} className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
          ...
          <div className="grid lg:grid-cols-3 gap-6">{/* Sales chart */}
            <div className="lg:col-span-2 ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={salesChartData} barSize={28}>
                  ...
                  <Bar dataKey="revenue" fill="var(--ap-primary)" radius={6} background={{ fill: "#EFF4FB" }} />
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
              ...
              <div className="space-y-3">{categoryData.map(c => (
                  <div key={c.name}>
          ...
          <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={conversionData}>
                ...
                <Line type="monotone" dataKey="rate" stroke="var(--ap-primary)" strokeWidth={2} dot={{ fill: "var(--ap-primary)", r: 4 }} connectNulls name={t('conversionRate')} />
                <Line type="monotone" dataKey="orders" stroke="#22C55E" strokeWidth={2} dot={{ fill: "#22C55E", r: 4 }} connectNulls name={t('ordersCount')} />
              </LineChart>
            </ResponsiveContainer>
          </div>{/* Top products */}
          <div className="ap-card bg-white rounded-xl border border-[#f0f0f0] p-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#f0f0f0] text-xs text-[#8f9294] font-semibold">{[t('product').toUpperCase(), "SKU", t('sold').toUpperCase(), t('revenue').toUpperCase(), t('vsLastMonth').toUpperCase()].map(h => (
