...
export default function AffiliateCommissionsPage() {
 ...
 useEffect(() => { fetch("/api/payouts").then(r => r.json()).then(d => setPayouts(Array.isArray(d) ? d : [])).catch(() => {}); }, []);
 const affPayouts = payouts.filter((p: any) => p.type === "affiliate");
 const realTotal = affPayouts.reduce((s: number, p: any) => s + (p.amount || 0), 0);
 const realPending = affPayouts.filter((p: any) => p.status !== "PAID").reduce((s: number, p: any) => s + (p.amount || 0), 0);
 const realPaid = realTotal - realPending;

 const totalPending = realPending;
 const totalPaid = realPaid;

 const handleCopy = () => {
 navigator.clipboard?.writeText(String(realPaid));
 setCopied(true);
 ...
 <button onClick={() => window.print()} className="ap-btn flex items-center gap-2 px-4 py-2 border border-[#e5e5e5] rounded-lg text-sm text-slate-600 hover:bg-[#f8f8fa]">Xuất Excel</button>
 </div>
 <div className="px-6 flex border-t border-[#f0f0f0]">{[["history", "Lịch sử giao dịch"], ["chart", (lang === "zh" ? "佣金图表" : "Biểu đồ hoa hồng")], ["request", "Yêu cầu rút tiền"]].map(([key, label]) => (
 ...
 { label: "Hoa hồng tháng này", value: fp(realTotal), color: "var(--ap-primary)", badge: (lang === "zh" ? "实数据" : "Từ payouts") },
 { label: "Hoa hồng thụ động (từ CTV)", value: fp(5800000), color: "#8B5CF6", badge: "12 CTV đang hoạt động" },
 { label: "Đang chờ thanh toán", value: fp(totalPending), color: "#F59E0B", badge: "Kỳ 20/10" },
 { label: "Tổng đã nhận tháng này", value: fp(totalPaid), color: "#22C55E", badge: "Đã thanh toán" },
 ].map(s => (
 ...
 <tbody>{commissionHistory.map((c, i) => (
 ...
 <YAxis hide />
 <Tooltip formatter={(v) => [`${Number(v).toLocaleString("vi-VN")} Triệu`, ""]} contentStyle={{ borderRadius: "8px" }} />
 ...
 {activeTab === "request" && (
 <div className="max-w-4xl mx-auto ap-card bg-white rounded-xl border border-[#f0f0f0] p-6">
 <h2 className="font-bold text-[#44494d] mb-5">{t("requestWithdraw")}</h2>

 <div className="p-4 rounded-xl mb-5" style={{ background: "#EFF4FB", border: "1px solid #DBEAFE" }}>
 ...
 <button className="w-full py-3.5 rounded-xl text-white font-bold" style={{ background: "var(--ap-primary)" }}>Gửi yêu cầu rút tiền
 </button>
 </div>
 </div>)}
 </div>
