...
function ProductsInner() {
  ...
  const brands = [...new Set(products.map((p: any) => p.brand).filter(Boolean))].sort() as string[];
  ...
  const filtered = products.filter(p =>
    ...
    if (sort === "rating") return (b.rating - a.rating) || ((b.reviewCount ?? b.reviews ?? 0) - (a.reviewCount ?? a.reviews ?? 0));
    return (b.sold ?? b.reviews ?? 0) - (a.sold ?? a.reviews ?? 0);
  });

  // Phân trang lưới sản phẩm — tránh trang dài
  const PAGE_SIZE = 12;
  const [page, setPage] = useState(1);
  useEffect(() => { setPage(1); }, [search, selectedCat, selectedSub, selectedSubSub, selectedType, selectedBrand, selectedOrigin, priceRangeIdx, sort]);
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: "#f8f8fa" }}>
      ...
      {/* Breadcrumb */}
      <div className="ap-container py-3 text-sm text-[#8f9294]">
      ...
      <div className="ap-container pb-12 grid lg:grid-cols-4 gap-6">
        {/* Sidebar filter */}
        <aside className="lg:col-span-1">
          <div className="bg-white rounded-xl border border-[#e5e5e5] p-4 sticky top-20 space-y-5 z-40 max-h-[calc(100vh-6rem)] overflow-y-auto custom-scrollbar">
                  ...
                  <Link key={prod.id} href={`/products/${prod.id}`}
                    ...
                    <div className="p-3 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-1 mb-2">
                        <div className="flex">
                          {[1,2,3,4,5].map(i => {
                            const rc = prod.reviewCount ?? prod.reviews ?? 0;
                            const r = rc > 0 ? (prod.rating ?? 0) : 0;
                            return <span key={i} className="text-xs" style={{ color: i <= Math.round(r) ? "#FBBF24" : "#E2E8F0" }}>★</span>;
                          })}
                        </div>
